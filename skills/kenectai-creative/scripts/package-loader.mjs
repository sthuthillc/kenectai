...
//   • requires an interactive y/N (or an explicit $KENECT_SKILL_BOOTSTRAP_DEPS=1)
...
const VERSION_OVERRIDE_ENV = "KENECT_SKILL_PKG_VERSION";
const BOOTSTRAP_ENV = "KENECT_SKILL_DEPS_BOOTSTRAPPED";
const BOOTSTRAP_CONFIRM_ENV = "KENECT_SKILL_BOOTSTRAP_DEPS";
const NODE_MODULES_ENV = "KENECT_SKILL_NODE_MODULES";
...
export function kenectaiPackageSpec(packageName) {
  const override = process.env[VERSION_OVERRIDE_ENV]?.trim();
  if (override) return `${packageName}@${override}`;

  const version = readBundledKenectaiVersion();
  if (version) return `${packageName}@${version}`;

  // Global skill installs (e.g. ~/.claude/skills) have no kenectai package.json
  ...
  process.stderr.write(
    [
      `kenectai: could not determine the bundled version for ${packageName}; using @latest.`,
...
function resolvePackageEntry(packageName) {
  ...
  for (const base of bases) {
    ...
    try {
      return createRequire(join(normalized, "__kenectai_skill_loader__.cjs")).resolve(
        packageName,
      );
    } catch {
...
function readBundledKenectaiVersion() {
...
function readPackageVersion(packageJsonPath) {
  try {
    ...
    if (manifest.name === "kenectai" || manifest.name === "@kenectai/cli") {
...
function bootstrapWithNpmInstall(packageNames) {
  const installRoot = mkdtempSync(join(tmpdir(), "kenectai-skill-deps-"));
