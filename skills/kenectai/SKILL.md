...
metadata: { "tags": "read-first, video, animation, router, kenectai, intent-routing" }
---

# KENECT AI — start here

KENECT AI **renders video from HTML** — a composition is an HTML file whose DOM declares timing with `data-*` attributes, whose animation runtime is seekable, and whose media playback is owned by the framework. The full authoring contract lives in `/kenectai-core`; read it before writing composition HTML.
...
| You want to…                                                                                                                                                             | Skill                    |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------ |
| **Author / edit an HTML composition** — the `data-*` contract, clips, tracks, sub-compositions, variables                                                                | `/kenectai-core`      |
| **Animate** — atomic motion, scene blueprints, transitions, runtime adapters (GSAP / Lottie / Three.js / Anime.js / CSS / WAAPI / TypeGPU)                               | `/kenectai-animation` |
| **Author seek-safe keyframes** — GSAP timelines, CSS keyframes, Anime.js, WAAPI, FLIP, paths, masks, SVG morph/draw, 3D depth, plus `kenectai keyframes` diagnostics  | `/kenectai-keyframes` |
| **Creative direction** — `frame.md` / `design.md`, palettes, typography, narration, beat planning, audio-reactive                                                        | `/kenectai-creative`  |
...
| **CLI dev loop** — init, lint, check, snapshot, preview, render, publish, doctor                                                                                         | `/kenectai-cli`       |
| **Install registry blocks / components** (`kenectai add`)                                                                                                             | `/kenectai-registry`  |
...
| `/remotion-to-kenectai` | **Porting an existing Remotion (React) composition** to KENECT AI (migration, not creation)                                                                                                                                                       |
...
```bash
npx @kenectai/cli skills update <workflow-name>
```
...
- **Core set** — this router, the `kenectai-*` domain skills, and `media-use`. `npx @kenectai/cli init` (which every creation workflow runs when scaffolding) checks GitHub and refreshes the core set plus anything else already installed. It never _expands_ the install — workflow skills you haven't used are not pulled. Re-running init on an up-to-date machine is a no-op; offline (or rate-limited) it degrades gracefully and never hard-fails. The `--skip-skills` flag is currently neutered (a temporary measure while the skills.sh registry catches up); CI/tests opt out via the `KENECT_SKIP_SKILLS=1` env var.
...
- **Check:** `npx @kenectai/cli skills check` (add `--json` for a machine-readable verdict; exits non-zero when anything installed is outdated or the core set is incomplete — workflow skills not yet installed are reported as _available on demand_, not as a failure).
- **Update:** `npx @kenectai/cli skills update` — refreshes the core set plus everything installed to the latest, and removes skills no longer published. Without names it never installs workflows you haven't used; naming skills (`skills update <name…>`) additionally installs those.
- **Full set, explicitly:** `npx @kenectai/cli skills` (or `npx skills add sthuthillc/kenectai --all`).
...
- **Output:** a KENECT AI composition (any length / format) via the original flow: design system → prompt expansion → plan → layout-before-animation → build (delegating to the `kenectai-`\* skills) → validate.
