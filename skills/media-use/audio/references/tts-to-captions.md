...
```bash
npx @kenectai/cli tts script.txt --provider heygen --output narration.wav --words narration.words.json
```
...
```bash
npx @kenectai/cli tts script.txt --voice af_heart --output narration.wav
npx @kenectai/cli transcribe narration.wav --model small.en   # voice af_heart is American English
```
