...
```bash
python3 skills/kenectai-creative/scripts/extract-audio-data.py narration.wav --fps 30 --bands 16 -o audio-data.json
```
...
**Never do:** equalizer bars, spectrum analyzers, waveform displays, strobing, rainbow color cycling. The audio provides _timing and intensity_; the visual vocabulary still comes from the brand. See `skills/kenectai-creative/references/audio-reactive.md` for the full API and anti-patterns.
