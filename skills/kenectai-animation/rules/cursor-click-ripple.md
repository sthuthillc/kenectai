...
- `/kenectai-animation` — timeline + tween API reference (eases, stagger, `immediateRender`, etc.)
- `/kenectai-core` — composition wiring (`data-*` attributes, scene structure, registration contract)
- `/kenectai-cli` — `kenectai lint` to verify the registry key + duration match
