...
- `/kenectai-animation` — `out`-family easing, proxy-driven `onUpdate` attribute tweens, and locked-envelope coordination (`../../adapters/gsap-easing-and-stagger.md`)
- `/kenectai-creative` — `references/typography.md` (embedded display face for a text streak), `references/video-composition.md` (solid field behind the smear)
- `/kenectai-core` — composition wiring, determinism (finite tweens, no `Math.random`)
- `/kenectai-cli` — `kenectai lint` / `kenectai check` (validate catches a missing `#streak-blur` node or an unreferenced filter)
