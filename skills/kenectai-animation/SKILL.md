...
For the composition contract (data attributes, sub-compositions, determinism) see `kenectai-core`.
...
**Prerequisite: `kenectai-core` → Non-Negotiable Rules** (single paused timeline, `data-duration` governs length, no `Math.random` / `Date.now` / `performance.now`, no `repeat: -1`, no `gsap.set` on later-scene clips, no `display` / `visibility` animation, no timeline construction inside `async` / `setTimeout` / `Promise`). Don't restate those here.
...
```bash
node skills/kenectai-animation/scripts/animation-map.mjs <composition-dir> \
  --out <composition-dir>/.kenectai/anim-map
```
...
`animation-map.mjs` resolves helper packages from the current project first, then can bootstrap the bundled KENECT AI package version. Set `KENECT_SKILL_PKG_VERSION=<version>` only when running the skill outside the bundled CLI/skill install and you need to pin that bootstrap version explicitly.

## See Also

- `kenectai-core` — composition structure, data attributes, sub-compositions, deterministic render contract
- `kenectai-creative` — palettes, typography, narration, beat planning (non-animation creative direction)
- `kenectai-cli` — `npx @kenectai/cli lint / validate / inspect / preview / render`
