...
Each entry has `name` and `type` (`kenectai:example`, `kenectai:block`, or `kenectai:component`).
...
| `type`                 | string   | yes      | `kenectai:block` or `kenectai:component` |
...
WebGPU + html-in-canvas frosted-glass surfaces. **Require Brave / Chrome canary** with WebGPU enabled — set `PRODUCER_HEADLESS_SHELL_PATH` to point at the browser; engine auto-passes `--enable-unsafe-webgpu`. See `/kenectai-animation` → `adapters/typegpu.md`.
...
HTML-in-canvas + WebGL composition blocks. See `/kenectai-animation` → `adapters/three.md` and `adapters/html-in-canvas-patterns.md` for the underlying APIs.
