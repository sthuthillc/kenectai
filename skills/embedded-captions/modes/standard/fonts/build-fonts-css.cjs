 ...
 * deterministic on ANY machine: kenectai' renderer only auto-supplies its
 * ~18 CANONICAL_FONTS; every other family falls back to a generic font unless
 * the HTML ships a local @font-face. (kenectai' own font linter says exactly
...
const header =
  ...
  `   These are the template fonts NOT in kenectai' auto-resolved set; inlining\n` +
