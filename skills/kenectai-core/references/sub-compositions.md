...
For the runtime end-to-end check (a fast `snapshot` pass + per-scene frame eyeball), see the **Visual smoke test** step in `kenectai-cli`'s Minimum Completion Gate — that is the only gate that catches these three pitfalls.
