---
name: kenectai-core
description: The KENECT AI composition contract — build one renderable project. Use for composition structure, the `data-*` timing attributes, `class="clip"`, tracks, sub-compositions, variables, framework-owned media playback, deterministic-render rules, and validation. Also covers Tailwind projects and the STORYBOARD.md / SCRIPT.md plan formats. Read before writing composition HTML.
---

# KENECT AI Core

KENECT AI renders video from HTML. A composition is an HTML file whose DOM declares timing with `data-*` attributes, whose animation runtime is seekable, and whose media playback is owned by the framework.

This skill is the **technical contract** — how to build one kenectai project. The body below is the build guide; per-topic detail lives in `references/` (index next), read on demand. Other concerns live in the sibling domain skills — `kenectai-animation`, `kenectai-creative`, `media-use`, `kenectai-cli`, `kenectai-registry`. The capability map in `/kenectai` says what each one covers.
...
For animation runtime specifics (GSAP API, Lottie, Three.js, etc.) go to `kenectai-animation` → `adapters/<runtime>.md`.
...
Use `kenectai-cli` for command details

- [ ] `npx @kenectai/cli check` passes (0 findings across lint, runtime, layout, motion, and contrast)
- [ ] Projects with sub-compositions: `npx @kenectai/cli snapshot --at <midpoints>` and eyeball each frame
- [ ] `npx @kenectai/cli preview` for review (the user can edit anything in Studio's timeline)
- [ ] `npx @kenectai/cli render` only after the user approves
