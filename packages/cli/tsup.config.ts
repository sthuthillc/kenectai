...
var __dirname = __hf_dirname(__filename);`,
  ...
  external: [
    ...
    // @kenectai/aws-lambda being a `dependencies` entry in package.json.
    "@kenectai/aws-lambda",
    "@kenectai/aws-lambda/sdk",
    // Same treatment for the GCP adapter: the cloudrun subverb files
    // dynamic-import `@kenectai/gcp-cloud-run/sdk` only when the user runs
    ...
    "@kenectai/gcp-cloud-run/sdk",
  ],
  noExternal: [
    "@kenectai/core",
    "@kenectai/parsers",
    "@kenectai/studio-server",
    "@kenectai/lint",
    "@kenectai/producer",
    "@kenectai/engine",
    "@clack/prompts",
  ...
  esbuildOptions(options) {
    options.alias = {
      "@kenectai/producer": resolve(__dirname, "../producer/src/index.ts"),
      // esbuild's alias map treats `@kenectai/producer` as a file path
      // and would otherwise resolve `@kenectai/producer/distributed`
      ...
      "@kenectai/producer/distributed": resolve(__dirname, "../producer/src/distributed.ts"),
      // Same reason: the lambda CLI imports `@kenectai/aws-lambda/sdk`,
      // which would resolve to `../aws-lambda/src/index.ts/sdk` without
      // an explicit subpath alias. The SDK subpath has its own barrel.
      "@kenectai/aws-lambda/sdk": resolve(__dirname, "../aws-lambda/src/sdk/index.ts"),
      // Same for the GCP adapter's SDK subpath barrel.
      "@kenectai/gcp-cloud-run/sdk": resolve(__dirname, "../gcp-cloud-run/src/sdk/index.ts"),
      // hf#677 follow-up: the shader-blend worker imports from
      // `@kenectai/engine/shader-transitions` (subpath export) — a
      // standalone TS file with zero internal imports that survives the
      // worker_thread loader boundary.
      "@kenectai/engine/shader-transitions": resolve(
        __dirname,
