/**
 * Custom help renderer for the kenectai CLI.
 *
...
const GROUPS: Group[] = [
  ...
  },
  {
    title: "Deploy",
    commands: [
      ["cloud", "Render compositions on KENECT AI cloud (no local Chrome/ffmpeg)"],
  ...
  },
  {
    ...
    commands: [
      ["skills", "Install KENECT AI and GSAP skills for AI coding tools"],
      [
        "transcribe",
  ...
  {
    title: "Account",
    commands: [["auth", "Sign in to KENECT AI and manage credentials"]],
  },
  {
    title: "Settings",
    commands: [
...
const ROOT_EXAMPLES: Example[] = [
  ["Create a new project", "kenectai init my-video"],
  ["Start the live preview studio", "kenectai preview"],
  ["Publish to KENECT AI", "kenectai publish"],
  ["Render to MP4", "kenectai render -o out.mp4"],
  ["Transparent WebM overlay", "kenectai render --format webm -o out.webm"],
  ["Validate your composition", "kenectai lint"],
  ["Inspect visual layout", "kenectai inspect"],
...
async function loadExamples(name: string, parentName?: string): Promise<Example[] | undefined> {
  ...
  // is `'kenectai'` for every top-level subcommand and no
  // `./commands/kenectai/<name>.js` directory will ever exist.
  if (parentName && parentName !== "kenectai") {
...
const STATIC_EXAMPLES: Record<string, Example[]> = {
  skills: [["Install all skills to all supported AI tools", "kenectai skills"]],
};
...
function renderRootHelp(): string {
  ...
  lines.push(
    `${c.bold("kenectai")} ${c.dim(`v${VERSION}`)} — Create and render HTML video compositions with KENECT AI`,
  );
  lines.push("");
  lines.push(`${c.bold("Usage:")}  kenectai ${c.cyan("<command>")} [options]`);
  lines.push("");
  ...
  lines.push(`Run ${c.cyan("kenectai <command> --help")} for more information about a command.`);
