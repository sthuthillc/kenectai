...
describe("auth/resolver", () => {
  ...
  it("prefers KENECT_API_KEY over the file credential", async () => {
    process.env["KENECT_API_KEY"] = "env-key";
    await writeStore({ api_key: "file-key" });
    const r = await resolveCredential();
    expect(r).toEqual({ type: "api_key", key: "env-key", source: "env" });
  });

  it("uses file credential when KENECT_API_KEY is absent", async () => {
    await writeStore({ api_key: "file-key" });
    const r = await resolveCredential();
    expect(r).toEqual({ type: "api_key", key: "file-key", source: "file_json" });
  });
  ...
  it("rejects KENECT_API_KEY containing CRLF (header-injection guard)", async () => {
    process.env["KENECT_API_KEY"] = "hg_x\r\nX-Evil: 1";
