/**
 * OAuth 2.0 + PKCE driver for the KENECT AI public OAuth flow.
 *
 * Entry points:
 ...
 *
 * All three endpoints are served by the kenect-api Cloud Run service
 * (`packages/kenect-api/src/oauthServer.ts`):
 *   - **Authorize** — GET /oauth/authorize
 *     Renders the login/consent screen; session cookie lives on the
 *     same origin, so the whole flow stays on one host.
 *   - **Token** — POST /v1/oauth/token
 *   - **Revoke** — POST /v1/oauth/revoke
 *
 * KENECT AI OAuth is configurable because this standalone CLI may point at
 * staging, production, or a local auth proxy while the product backend settles.
 *
 * Overrides:
 *   - `KENECT_OAUTH_AUTHORIZE_URL`
 *   - `KENECT_OAUTH_TOKEN_URL`
 *   - `KENECT_OAUTH_REVOKE_URL`
 *   - `KENECT_OAUTH_CLIENT_ID`
 *
 ...
 * `KENECT_OAUTH_CLIENT_ID` env var. Empty string means "not
 * configured" — `resolveClientId()` errors cleanly with a pointer
 * at `--api-key`.
 */
const DEFAULT_CLIENT_ID = "KajA15NGKvOCEG7bXdyzqJFs";
const DEFAULT_SCOPES = "openid profile email";

// Endpoint defaults — see file-header comment for why these straddle two
// hosts. Each is independently overridable.
// All three live on the kenect-api service behind the api.kenectai.com
// Cloud Run domain mapping (the consent screen is served by the API
// itself, so no separate app host is involved).
const DEFAULT_AUTHORIZE_URL = "https://api.kenectai.com/oauth/authorize";
const DEFAULT_TOKEN_URL = "https://api.kenectai.com/v1/oauth/token";
const DEFAULT_REVOKE_URL = "https://api.kenectai.com/v1/oauth/revoke";

function authorizeEndpoint(): string {
  return process.env["KENECT_OAUTH_AUTHORIZE_URL"] || DEFAULT_AUTHORIZE_URL;
}
function tokenEndpoint(): string {
  return process.env["KENECT_OAUTH_TOKEN_URL"] || DEFAULT_TOKEN_URL;
}
function revokeEndpoint(): string {
  return process.env["KENECT_OAUTH_REVOKE_URL"] || DEFAULT_REVOKE_URL;
}
...
export function resolveClientId(): string {
  const override = process.env["KENECT_OAUTH_CLIENT_ID"];
...
async function exchangeCodeForTokens(args: {
  ...
  if (res.status === 400 || res.status === 401) {
    ...
    // a bare "KENECT AI API error (400)".
