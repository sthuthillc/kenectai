/**
 * Minimal typed HTTP client for KENECT AI endpoints needed by the auth
 ...
 * disproportionate.
 *
 * Reads `KENECT_API_URL` (default deployed KENECT API) so dev
 ...
 *
 * The backend `/v3/users/me` accepts both.
 */
...
const DEFAULT_BASE_URL = "https://api.kenectai.com";
export const KENECT_CLI_SOURCE_HEADER = "X-Kenect-Source";
export const KENECT_CLI_SOURCE = "cli";

export function apiBaseUrl(): string {
  const override = process.env["KENECT_API_URL"];
 ...
 * against the KENECT API. Modelling them as nested objects so the row
...
export interface AuthClientOptions {
  /** Override base URL (otherwise `KENECT_API_URL` / default). */
  baseUrl?: string;
