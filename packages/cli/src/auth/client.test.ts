...
describe("auth/client", () => {
  const original = process.env["KENECT_API_URL"];

  beforeEach(() => {
    delete process.env["KENECT_API_URL"];
  });

  afterEach(() => {
    if (original !== undefined) process.env["KENECT_API_URL"] = original;
    else delete process.env["KENECT_API_URL"];
  });

  it("apiBaseUrl defaults to the deployed KENECT API", () => {
    expect(apiBaseUrl()).toBe("https://api.kenectai.com");
  });

  it("apiBaseUrl honors KENECT_API_URL and strips trailing slash", () => {
    process.env["KENECT_API_URL"] = "https://api.dev.kenectai.com/";
    expect(apiBaseUrl()).toBe("https://api.dev.kenectai.com");
  });
  ...
  it("buildAuthHeaders uses Bearer for oauth", () => {
    ...
    expect(buildAuthHeaders(cred)).toEqual({
      ...
      [KENECT_CLI_SOURCE_HEADER]: KENECT_CLI_SOURCE,
    });
  });
