...
export const ErrNotConfigured = () =>
  new AuthError(
    "NOT_CONFIGURED",
    "No KENECT AI credentials found",
    "Run `kenectai auth login` to sign in.",
  );
...
export const ErrInvalidStore = (detail: string) =>
  new AuthError(
    ...
    "Delete ~/.kenectai/credentials and run `kenectai auth login` to re-create it.",
  );

export const ErrUnauthenticated = (detail?: string) =>
  new AuthError(
    "UNAUTHENTICATED",
    detail ? `KENECT AI rejected the credential: ${detail}` : "KENECT AI rejected the credential",
    "Run `kenectai auth login` to re-authenticate.",
  );

export const ErrApi = (status: number, detail: string) =>
  new AuthError("API_ERROR", `KENECT AI API error (${status}): ${detail}`);

export const ErrOAuthNotConfigured = () =>
  new AuthError(
    "OAUTH_NOT_CONFIGURED",
    "OAuth client is not configured",
    "Set KENECT_OAUTH_CLIENT_ID, or run `kenectai auth login --api-key`.",
  );
