...
describe("auth/store", () => {
  ...
  it("accepts a legacy plaintext key of any KENECT-compatible key format", async () => {
    // KENECT AI keys can come in multiple formats (`sk_V2_…`, `hg_…`,
  ...
  // The credentials file can be shared by multiple KENECT AI tools. If this CLI
  ...
  it("preserves another tool's `user` block when this CLI rewrites only the credential", async () => {
    // The exact cross-CLI data-loss scenario: another KENECT tool wrote a
    // `user` block; kenectai updates the api_key and must not drop it.
    await fs.writeFile(
      path,
      JSON.stringify({
        api_key: "hg_old",
