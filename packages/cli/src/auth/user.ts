 ...
 * the shared `~/.kenectai/credentials` file.
 *
 ...
 *
 * This file can be shared by multiple KENECT AI tools. Persisting here goes
 * through `readStore` / `writeStore`, which round-trip every unrecognized key
 * so saving our user block never clobbers a key another tool wrote.
 */
