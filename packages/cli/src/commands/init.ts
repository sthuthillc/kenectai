...
export const examples: Example[] = [
  ["Create a project with the interactive wizard", "kenectai init my-video"],
  ["Pick a starter example", "kenectai init my-video --example warm-grain"],
  ["Scaffold a 4K project", "kenectai init my-video --resolution 4k"],
  ["Scaffold a portrait video", "kenectai init my-video --resolution portrait"],
  ["Start from an existing video file", "kenectai init my-video --video clip.mp4"],
  ["Start from an audio file", "kenectai init my-video --audio track.mp3"],
  ["Scaffold with Tailwind CSS", "kenectai init my-video --example blank --tailwind"],
  ["Non-interactive mode (for CI or AI agents)", "kenectai init my-video --non-interactive"],
  [
    "Opt out of the GitHub skills check (CI/tests only)",
    "KENECT_SKIP_SKILLS=1 kenectai init my-video --non-interactive",
  ],
];
import {
  existsSync,
...
function toPackageName(projectName: string): string {
  ...
  return normalized || "kenectai-project";
}

function getKenectPackageSpecifier(): string {
  return VERSION === "0.0.0-dev" ? "kenectai" : `@kenectai/cli@${VERSION}`;
}

function kenectScript(command: string): string {
  return `npx --yes ${getKenectPackageSpecifier()} ${command}`;
}

function buildPackageScripts(): Record<string, string> {
  return {
    dev: kenectScript("preview"),
    check: kenectScript("check"),
    render: kenectScript("render"),
    publish: kenectScript("publish"),
  };
}
...
async function scaffoldProject(
  ...
  // Write kenectai.json so `kenectai add` knows which registry to use
  // and where to drop block/component files. Overwritten only if absent.
  if (!existsSync(resolve(destDir, "kenectai.json"))) {
 ...
 * set. Guarantees the core set (the `/kenectai` entry router + shared
 ...
 * workflow is triggered (`kenectai skills update <name>`, which the router
...
async function keepSkillsCurrent(destDir: string): Promise<void> {
  ...
  try {
    ...
    if (result.presenceOnly) {
      ...
      console.log(
        c.dim("Skills freshness unverified — run `npx kenectai skills update` when online."),
      );
...
export default defineCommand({
  ...
  args: {
    ...
    "skip-skills": {
      type: "boolean",
      description:
        "[temporarily ignored] init always checks AI skills against GitHub while the skills.sh registry catches up; set KENECT_SKIP_SKILLS=1 to opt out (CI/tests)",
    },
    tailwind: {
      type: "boolean",
  ...
  async run({ args }) {
    if (args.template !== undefined) {
      ...
      console.error(
        c.error(
          `The --template flag was renamed to --example. Example:\n  npx kenectai init ${args.name ?? "my-video"} --example "${args.template}"`,
        ),
      );
      process.exit(1);
    }
    ...
    // @kenectai/cli@latest`). CI and unit tests still opt out via the
    // KENECT_SKIP_SKILLS=1 env var, which the agent/user CLI path never sets.
    // Revert to `args["skip-skills"] === true` once skills.sh catches up.
    const skipSkills = process.env.KENECT_SKIP_SKILLS === "1";
    ...
    if (!interactive) {
      ...
      if (skipSkills) {
        ...
        console.log(`     ${c.accent("npx kenectai skills update")}`);
      } else {
        console.log(
      ...
      console.log(
        `     ${c.dim('"Using /kenectai, create a 15-second intro about [your topic]"')}`,
      );
      console.log(`     ${c.dim("More patterns: docs.kenectai.com/guides/prompting")}`);
      console.log();
      ...
      console.log();
      console.log(`  ${c.dim("Full docs: docs.kenectai.com")}`);
      return;
    }
    ...
    // -----------------------------------------------------------------------
    printBanner();
    clack.intro("Create a new KENECT AI project");
