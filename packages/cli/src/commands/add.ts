...
export const examples: Example[] = [
  ["Add a block to the current project", "kenectai add claude-code-window"],
  ["Add a component effect", "kenectai add shader-wipe"],
  ["Add all HTML-in-Canvas blocks", "kenectai add html-in-canvas"],
  ["Add all caption blocks", "kenectai add captions"],
  ["Target a specific project directory", "kenectai add shader-wipe --dir ./my-video"],
  ["Skip the clipboard copy (CI/headless)", "kenectai add shader-wipe --no-clipboard"],
];

import { existsSync } from "node:fs";
import { resolve, relative } from "node:path";
import { ITEM_TYPE_DIRS, type RegistryItem } from "@kenectai/core";
...
// user's `kenectai.json#paths` so a project can reshape its layout
...
export async function runAdd(opts: RunAddArgs): Promise<RunAddResult> {
  ...
  if (item.type === "hyperframes:example") {
    throw new AddError(
      `"${item.name}" is an example — use \`kenectai init <dir> --example ${item.name}\` instead.`,
      "example-type",
    );
  }
