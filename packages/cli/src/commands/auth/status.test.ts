...
describe("buildUnconfiguredLines — interactive (TTY / agent-driven)", () => {
  ...
  it("makes browser OAuth the kenectai path", () => {
    expect(text).toContain("kenectai auth login");
  ...
  it("shows only zero-install `npx kenectai` paths", () => {
    expect(text).toContain("npx kenectai auth login");
    expect(text).toContain("npx kenectai auth login --api-key");
  });

  it("offers the --api-key path as a secondary option", () => {
    expect(text).toContain("kenectai auth login --api-key");
  });
});
...
describe("buildUnconfiguredLines — non-interactive (CI / piped)", () => {
  ...
  it("points at KENECT_API_KEY and the local fallback", () => {
    expect(text).toContain("KENECT_API_KEY");
  ...
  it("recommends auth login and reports the local fallback", () => {
    for (const ctx of [INTERACTIVE, NON_INTERACTIVE]) {
      ...
      expect(payload).toMatchObject({
        ...
        recommended_action: "npx kenectai auth login",
        fallback: "local",
      });
    }
  });
