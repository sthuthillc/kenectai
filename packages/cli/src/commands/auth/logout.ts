/**
 * `kenectai auth logout` — remove the credential file. With
 * `--keep-api-key`, only the OAuth block is cleared (no-op for
 * API-key-only stores).
 *
 * Env-only credentials (`KENECT_API_KEY`) can't
...
export default defineCommand({
  meta: { name: "logout", description: "Remove the stored KENECT AI credential" },
  args: {
    "keep-api-key": {
      type: "boolean",
...
async function ensureConfirmed(yes: boolean, keepApiKey: boolean): Promise<boolean> {
  ...
  const prompt = keepApiKey
    ? `This will sign out of any active OAuth session on this machine (~/.kenectai lives at ${configDir()}). Continue? [y/N] `
    : `This will sign out of KENECT AI on this machine (~/.kenectai lives at ${configDir()}). Continue? [y/N] `;
