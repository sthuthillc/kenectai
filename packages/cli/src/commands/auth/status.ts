/**
 * `kenectai auth status` — print the active credential's source,
 ...
 * registration guidance led by `kenectai auth login` — sign-in and
...
export default defineCommand({
  meta: { name: "status", description: "Show the active KENECT AI credential" },
  args: {
    json: {
      type: "boolean",
 ...
 * `hasKenect` is false here by construction — we only reach this when no
 ...
 * credentials. An env credential (`KENECT_API_KEY`)
...
const SOURCE_LABELS: Record<ResolvedCredential["source"], string> = {
  env: "env (KENECT_API_KEY)",
  file_legacy: "file (~/.kenectai/credentials — legacy plaintext)",
  file_json: "file (~/.kenectai/credentials)",
};
