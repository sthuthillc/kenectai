...
const RECOMMENDED_ACTION = "npx kenectai auth login";
 ...
 * Both paths use `npx kenectai` (zero-install via npm): browser OAuth
 * (sign-in / sign-up) and `--api-key` both write `~/.kenectai`. Names the
 * local fallback so "no key" never reads as a failure, and never steers users
 * toward a per-repo `.env`. Mirrors the media-use skill's Preflight section.
 */
export function buildUnconfiguredLines(
  ...
  if (!ctx.interactive) {
    return [
      c.warn("Not signed in to KENECT AI (non-interactive)."),
      c.dim(
        "Set KENECT_API_KEY to use KENECT AI, or workflows fall back to local engines (Kokoro voice · MusicGen music).",
      ),
    ];
  }
  return [
    c.warn("Not signed in to KENECT AI — voice & music will use local engines (free, offline)."),
    "",
    "Sign in or sign up (browser OAuth, writes ~/.kenectai — no per-repo .env):",
    `  ${c.accent("npx kenectai auth login")}            ${c.dim("# browser sign-in / sign-up")}`,
    "",
    "Or paste an existing KENECT AI API key:",
    `  ${c.accent("npx kenectai auth login --api-key")}  ${c.dim("# paste at the prompt")}`,
    "",
