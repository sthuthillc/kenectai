...
const ENV_KEYS = ["KENECT_API_KEY", "KENECT_CONFIG_DIR"] as const;
...
describe("auth login --api-key rollback", () => {
  ...
  beforeEach(async () => {
    ...
    process.env["KENECT_CONFIG_DIR"] = dir;
  ...
  it("preserves an unknown/foreign top-level key across a successful re-login", async () => {
    // Cross-CLI invariant end-to-end: a key another KENECT AI tool (or a
    // future version) wrote must survive a kenectai login round-trip.
