/**
 * `kenectai auth login` — sign in to KENECT AI.
 *
...
export default defineCommand({
  meta: {
    name: "login",
    description: "Sign in to KENECT AI (OAuth by default; --api-key for long-lived keys)",
  },
  args: {
    "api-key": {
      type: "string",
 ...
 * `/v3/users/me` exposes no opaque user_id, so we key on the KENECT AI account
 ...
 * (Username is NOT a privacy win — usernames can be email-shaped
...
async function verifyAndReport(key: string): Promise<UserInfo | null> {
  ...
  try {
    ...
    if (isAuthError(err) && err.code === "UNAUTHENTICATED") {
      console.error(
        `${c.warn("KENECT AI rejected the API key.")}\n` +
          `  ${c.dim(err.message)}\n` +
          `Run ${c.accent("kenectai auth login --api-key")} again with a valid key.`,
      );
      return null;
    }
    throw err;
...
async function promptForKey(): Promise<string> {
  ...
  const value = await clack.password({
    message: "Enter KENECT AI API key",
    validate: (v) => {
