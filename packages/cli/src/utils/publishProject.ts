...
async function readErrorMessage(response: Response, fallback: string): Promise<string> {
  ...
  if (response.status === 403 && response.headers.get("cf-mitigated") === "challenge") {
    return "Publish upload was blocked before reaching KENECT AI. Please retry after staged uploads are available.";
  }
...
export function getPublishApiBaseUrl(): string {
  return (
    process.env["KENECT_PUBLISHED_PROJECTS_API_URL"] ||
...
async function publishProjectArchiveDirect(
  ...
  const response = await fetch(`${apiBaseUrl}/v1/kenectai/projects/publish`, {
    method: "POST",
    body,
    headers,
...
async function publishProjectArchiveStaged(
  ...
  const uploadResponse = await fetch(`${apiBaseUrl}/v1/kenectai/projects/publish/upload`, {
    method: "POST",
    body: JSON.stringify({
      file_name: fileName,
  ...
  const completeResponse = await fetch(`${apiBaseUrl}/v1/kenectai/projects/publish/complete`, {
    method: "POST",
    body: JSON.stringify({
