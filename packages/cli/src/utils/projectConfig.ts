/**
 * Read and write `kenectai.json` — the per-project config that tells
 * `kenectai add` which registry to pull items from and where to drop them
 * in the user's project tree.
 *
 * The file is created by `kenectai init` and optionally edited by users to
...
export const PROJECT_CONFIG_FILENAME = "kenectai.json";
const PROJECT_CONFIG_SCHEMA_URL = "https://docs.kenectai.com/schema/kenectai.json";
...
/** Read `kenectai.json` from a project directory. */
...
/** Write `kenectai.json` to a project directory. Overwrites if present. */
