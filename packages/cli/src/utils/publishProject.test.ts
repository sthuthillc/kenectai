...
describe("publishProjectArchive", () => {
  ...
  it("uploads through the staged publish flow and returns the stable project URL", async () => {
    ...
    try {
      ...
      expect(getPublishApiBaseUrl()).toBe("https://api.kenectai.com");
      ...
      expect(fetchMock).toHaveBeenNthCalledWith(
        1,
        "https://api.kenectai.com/v1/kenectai/projects/publish/upload",
      ...
      expect(fetchMock).toHaveBeenNthCalledWith(
        3,
        "https://api.kenectai.com/v1/kenectai/projects/publish/complete",
        expect.objectContaining({
          method: "POST",
          headers: { "content-type": "application/json" },
          signal: expect.any(AbortSignal),
  ...
  it("falls back to the legacy multipart endpoint when staged publish is not deployed", async () => {
    ...
    try {
      ...
      expect(fetchMock).toHaveBeenNthCalledWith(
        2,
        "https://api.kenectai.com/v1/kenectai/projects/publish",
        expect.objectContaining({
          method: "POST",
          headers: {},
          signal: expect.any(AbortSignal),
