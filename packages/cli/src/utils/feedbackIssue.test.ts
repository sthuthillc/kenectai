...
import { buildIssueUrl, KENECT_REPO_URL } from "./feedbackIssue.js";
...
describe("buildIssueUrl", () => {
  const base = {
    repoUrl: KENECT_REPO_URL,
    rating: 2,
    comment: "GSAP timeline froze on seek",
    repoPublicUrl: "https://app.kenectai.com/p/abc123",
  ...
  it("points at the repo /issues/new with the bug label", () => {
    ...
    expect(url.startsWith(`${KENECT_REPO_URL}/issues/new?`)).toBe(true);
  ...
  it("encodes the title and includes rating + repro URL in the body", () => {
    ...
    expect(body).toContain("https://app.kenectai.com/p/abc123");
  ...
  it("strips a trailing .git from the repo url", () => {
    const url = buildIssueUrl({
      ...base,
      repoUrl: "https://github.com/sthuthillc/kenectai.git",
    });
    expect(url.startsWith(`${KENECT_REPO_URL}/issues/new?`)).toBe(true);
  });
