/**
 * Poll a KENECT AI render until it reaches a terminal state.
 *
...
import type { KenectaiRenderDetail, KenectaiRenderStatus } from "./_gen/types.js";

export interface PollOptions {
  intervalMs?: number;
  maxWaitMs?: number;
  /** Called once per tick with the latest render state. */
  onTick?: (detail: KenectaiRenderDetail, elapsedMs: number) => void;
...
const TERMINAL_STATUSES: ReadonlySet<KenectaiRenderStatus> = new Set(["completed", "failed"]);

export class PollTimeoutError extends Error {
  readonly lastDetail: KenectaiRenderDetail;
  constructor(lastDetail: KenectaiRenderDetail, elapsedMs: number) {
    super(
...
export function isTerminal(status: KenectaiRenderStatus): boolean {
  return TERMINAL_STATUSES.has(status);
}

/**
 * Poll `GET /v3/kenectai/renders/{id}` until status is `completed` or
