...
description: "Render a composition on KENECT AI's hosted cloud — no local Chrome, no FFmpeg, no AWS to manage."
---

Render any KENECT AI composition on KENECT AI's managed cloud: the CLI zips your project, uploads it, runs the render on KENECT AI's infrastructure, and downloads the finished video. There's nothing to deploy and no Chrome or FFmpeg to install — you pay per credit.

```bash
kenectai auth login            # one-time sign-in
kenectai cloud render          # zip → upload → render → download
```
...
This is the zero-infra alternative to running your own renderer. If you'd rather own the compute, see [AWS Lambda](/deploy/aws-lambda), [GCP Cloud Run](/deploy/gcp-cloud-run), or the [Vercel, Cloudflare, or Modal templates](/guides/deploy). For local iteration during authoring, use [`kenectai render`](/guides/rendering).

## Authenticate

Cloud rendering needs a KENECT AI credential. Sign in once — the CLI stores it in `~/.kenectai/credentials` (mode `0600`), and the same credential drives every `cloud` subcommand.
...
The credential is **shared across KENECT AI tools** via `~/.kenectai/credentials` — sign in with one and the others pick up the session. Credentials resolve in this order (first match wins):

1. `KENECT_API_KEY` environment variable
2. `~/.kenectai/credentials` (unexpired OAuth session, else stored API key)

<Note>
  Point the CLI at a different backend with `KENECT_API_URL` (default `https://api.kenectai.com`). Use `kenectai auth refresh` to force-refresh an OAuth token before a long job; `kenectai auth logout` clears the stored credential. For the keys voice, music, and capture use across the skills — and the fully local fallback — see [Authentication & API keys](/guides/authentication).
</Note>

## How a cloud render flows

`kenectai cloud render` runs the whole pipeline end-to-end:

```
 Your machine                          KENECT AI cloud
┌─────────────────────────┐          ┌─────────────────────────────────┐
...
3. **Zip** the project (same ignore set as `kenectai publish`).
...
Cloud rendering supports [variables](/concepts/variables) — the same mechanism that powers templates everywhere else in KENECT AI. Declare `data-composition-variables` on your composition, then fill them at render time:
...
# From a file
kenectai cloud render --variables-file ./vars.json

# Fail fast on undeclared keys or wrong types
kenectai cloud render --variables '{"title":"Q4 Recap"}' --strict-variables
```
...
kenectai cloud render ./card-template

# 2. Re-render the same asset with new values (skips zip + upload).
kenectai cloud render --asset-id asst_abc123 --variables '{"name":"Ada"}'
kenectai cloud render --asset-id asst_abc123 --variables '{"name":"Linus"}'
```
...
```bash
kenectai cloud list                 # recent renders (--limit, --token, --all)
kenectai cloud get hfr_def456       # full detail + short-lived signed video_url
kenectai cloud delete hfr_def456    # soft-delete (--no-confirm to skip the prompt)
```
...
- **`kenectai render`** (local) — fastest iteration loop; use while authoring. See [Rendering](/guides/rendering).
- **`kenectai cloud render`** — zero-infra; HeyGen runs the render and you pay per credit. Use when you don't want to manage Chrome/FFmpeg/AWS.
- **`kenectai lambda render`** — bring-your-own-AWS distributed rendering with chunked parallelism. Use when you've already invested in AWS. See [AWS Lambda](/deploy/aws-lambda).
