...
Cloud rendering needs a HeyGen credential. Sign in once — the CLI stores it in `~/.kenectai/credentials` (mode `0600`), and the same credential drives every `cloud` subcommand.
