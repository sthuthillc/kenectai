...
description: How to add blocks and components to the KENECT AI registry.
---

Your agent already knows how to build video components. It writes HTML. KENECT AI renders it. The registry is the collection of everything that's been built — 52 blocks and counting.
...
Every block in the registry exists because someone needed it and built it. When you add a block, every KENECT AI user gets it with one command:

```bash
npx @kenectai/cli add instagram-follow
```

The registry grows, KENECT AI gets more useful, and your work ships to everyone.
...
Open an issue on [GitHub](https://github.com/sthuthillc/kenectai/issues) with a visual reference. Tag it `component-request`.
...
If you use Claude Code with KENECT AI skills:
...
<Steps>
  <Step title="Fork and create">
    Fork [sthuthillc/kenectai](https://github.com/sthuthillc/kenectai) and create your block directory:
    ```bash
