...
description: "Fast reference for video editors and creative people directing agents, cutting timing, tweaking layouts, previewing, and publishing KENECT AI projects."
---

Use this as a fast reference when you are directing agents, cutting timing, making visual layout tweaks, previewing, and sharing KENECT AI projects.

## The Fast Loop

```bash
npx @kenectai/cli init my-video --example blank
cd my-video
...
npx @kenectai/cli render --quality standard --output review.mp4
```

For fast iteration renders, use draft quality:

```bash
npx @kenectai/cli render --quality draft --output draft.mp4
```

For final delivery:

```bash
npx @kenectai/cli render --quality high --fps 30 --output final.mp4
```
...
Common KENECT AI project folders:
...
Run KENECT AI commands from the project root, where `index.html` lives. If you are not sure where you are, run `pwd` then `ls`. If you see `index.html`, you are in the right place.
...
| Command | Use it for |
| --- | --- |
| `npx @kenectai/cli init my-video` | Create a new project |
| `npx @kenectai/cli init my-video --example warm-grain` | Start from a visual template |
| `npx @kenectai/cli init my-video --video source.mp4` | Import video and generate captions from the source audio |
| `npx @kenectai/cli capture https://example.com` | Capture a website as source material for a video |
| `npx @kenectai/cli preview` | Open the live Studio preview |
| `npx @kenectai/cli lint` | Catch structural mistakes before preview or render |
| `npx @kenectai/cli check` | Run the composition in headless Chrome to catch runtime errors |
| `npx @kenectai/cli check` | Find text overflow and layout problems across the timeline |
| `npx @kenectai/cli snapshot --at 1,3,5` | Save PNG checks at exact timestamps |
| `npx @kenectai/cli render --output final.mp4` | Render the video |
| `npx @kenectai/cli publish` | Upload the project and get a shareable KENECT AI URL |
| `npx @kenectai/cli doctor` | Check Node.js, FFmpeg, Chrome, Docker, and other dependencies |
| `npx @kenectai/cli docs` | Open local CLI docs |
| `npx @kenectai/cli upgrade` | Check for a newer CLI version |
...
| Goal | Command |
| --- | --- |
| Fast iteration | `npx @kenectai/cli render --quality draft --output draft.mp4` |
| Review link | `npx @kenectai/cli render --quality standard --output review.mp4` |
| Final export | `npx @kenectai/cli render --quality high --fps 30 --output final.mp4` |
| Transparent overlay | `npx @kenectai/cli render --format webm --output overlay.webm` |
| Deterministic output | `npx @kenectai/cli render --docker --output final.mp4` |
...
```bash
npx @kenectai/cli publish ./my-video   # publish a specific folder
npx @kenectai/cli publish --yes        # skip the confirmation prompt in scripts
```
...
Use it when you want the agent to open the KENECT AI Studio preview, play or scrub the video, click timeline controls, inspect visible UI text, capture screenshots for review, or record proof of a tested flow.

For editor-facing changes, keep `npx @kenectai/cli preview` running, then have the agent use `agent-browser` against the local preview URL.

## Quick Fixes

| Problem | Command or check |
| --- | --- |
| Preview will not start | `npx @kenectai/cli doctor` |
| Port already in use | `npx @kenectai/cli preview --port 4567` |
| Render fails | `npx @kenectai/cli lint` then `npx @kenectai/cli check` |
| Need exact frame checks | `npx @kenectai/cli snapshot --at 1,2.5,5` |
| Text overflows in the frame | `npx @kenectai/cli check` |
| Final render is too slow | Try `--quality draft`, reduce image sizes, or lower `--fps` |
| Need to share editable project | `npx @kenectai/cli publish` |
