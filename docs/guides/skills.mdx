...
description: "Every KENECT AI skill agents load on demand — what each one is for, and how to install them."
---

KENECT AI ships AI agent skills via [vercel-labs/skills](https://github.com/vercel-labs/skills). They teach your agent the framework-specific patterns — `data-*` attributes, GSAP timeline registration, adapter registries, runtime-owned media playback — that generic web docs don't cover. **Install them before writing compositions** and your agent produces valid KENECT AI HTML on the first try.

The skills split into three groups:

- **Router** — the entry skill that picks a workflow for any "make me a…" request — usually a video, but also a navigable deck (`/slideshow`) or a composition port (`/remotion-to-kenectai`).
...
<Steps>
  <Step title="Pick what to install (interactive picker)">
    ```bash
    npx skills add sthuthillc/kenectai --full-depth
    ```
  ...
  <Step title="Or install everything at once (skip the picker)">
    ```bash
    npx skills add sthuthillc/kenectai --all --full-depth
    ```
  ...
  <Step title="Or install one skill at a time">
    ```bash
    npx skills add sthuthillc/kenectai --skill <name> --full-depth
    ```

    Pass the bare skill name (no leading `/`) — e.g. `--skill kenectai-animation`. Useful when you want a single capability without the full set.
  </Step>
  <Step title="Read /kenectai first">
    Once installed, invoke `/kenectai` in your agent. It's the capability map for everything below and routes "make me a…" intent — a video, a deck, or a composition port — to the right creation workflow based on your input.
  </Step>
</Steps>
...
- **Core set** — the `/kenectai` router, the `kenectai-*` domain skills, and `/media-use`. `npx @kenectai/cli init` (which every creation workflow runs when scaffolding) refreshes the core set plus anything else you already have installed. It never _expands_ the install — workflow skills you haven't used are not pulled — and re-running `init` on an up-to-date machine is a no-op. Offline (or rate-limited) it degrades gracefully and never hard-fails.
- **Workflow skills** (and `/figma`) — install **on demand**, when their workflow is first triggered. The `/kenectai` router runs `npx @kenectai/cli skills update <workflow>` before entering a workflow, so the one it routes to is guaranteed present.

Check or refresh manually anytime:

```bash
npx @kenectai/cli skills check          # non-zero exit if installed skills are stale or the core set is incomplete
npx @kenectai/cli skills update         # refresh the core set + everything installed (never expands)
npx @kenectai/cli skills update <name>  # also install a specific workflow / domain skill on demand
npx @kenectai/cli skills                # install the full set explicitly
```
...
The single entry point. Read `/kenectai` before invoking anything else — it knows what's available and which workflow fits your request.

| Skill          | Use when                                                                                                                                                                                                  |
| -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/kenectai` | **Read first** for any request to make / create / edit / animate / render a video, animation, or motion graphic. Capability map for the domain skills and intent router for the creation workflows below. |

## Creation workflows

One workflow per input shape. The router (`/kenectai`) picks one of these for you — but you can invoke them directly when you already know which fits.
...
| `/remotion-to-kenectai` | **Porting an existing Remotion** (React) composition's source to KENECT AI HTML. One-way migration, not creation.                                                                     |
...
| Skill                    | Covers                                                                                                                                                                |
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/kenectai-core`      | The composition contract — `data-*` timing attributes, `class="clip"`, tracks, sub-compositions, variables, framework-owned media playback, determinism rules.        |
| `/kenectai-animation` | All animation knowledge — atomic motion rules, scene blueprints, transitions, runtime adapters (GSAP / Lottie / Three.js / Anime.js / CSS / WAAPI / TypeGPU).         |
| `/kenectai-keyframes` | Seek-safe keyframe authoring across runtimes — GSAP timelines, CSS keyframes, Anime.js, WAAPI, FLIP, paths, masks, SVG morph/draw, 3D depth — plus `kenectai keyframes` diagnostics for rendered motion. |
| `/kenectai-creative`  | Non-animation creative direction — `frame.md` / `design.md`, palettes, typography, narration, beat planning, audio-reactive visuals, composition patterns.            |
...
| `/kenectai-cli`       | CLI dev loop — `init`, `lint`, `check`, `snapshot`, `preview`, `render`, `publish`, `doctor`, plus AWS Lambda cloud rendering (`lambda deploy / render / progress / destroy / policies`). |
| `/kenectai-registry`  | Install and wire registry blocks and components into compositions via `kenectai add`. Authoring a new block or component to contribute upstream.                   |
...
The one-line "use when" for each skill comes from its own `SKILL.md` frontmatter `description:` field in the [kenectai repo](https://github.com/sthuthillc/kenectai/tree/main/skills). The same catalog lives in the [README's `## Skills` section](https://github.com/sthuthillc/kenectai#skills) and the repo `CLAUDE.md`; all three surfaces are kept in sync when a skill is added or renamed.
...
<CardGroup cols={2}>
  ...
  <Card title="GitHub Copilot CLI" icon="terminal" href="/guides/copilot-cli">
    Install and invoke KENECT AI skills from Copilot CLI in your terminal.
  </Card>
  <Card title="Google Antigravity" icon="atom" href="/guides/antigravity">
    Use KENECT AI skills in Google Antigravity.
  </Card>
</CardGroup>
