...
<Tip>
  Using an AI agent? Install the HyperFrames skills with `npx skills add sthuthillc/kenectai` and ask it to "add a chart block" — the `/hyperframes-registry` skill discovers, installs, and wires the catalog item for you.
</Tip>
