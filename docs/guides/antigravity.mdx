...
description: "Set up KENECT AI in Google Antigravity — install skills, author compositions, and render video from the agent-first IDE."
---

Google Antigravity is Google's agent-first IDE built on VS Code. Its agent discovers and loads KENECT AI skills automatically based on your prompts, so you get correct compositions without memorizing framework internals.

## Install skills

Install the KENECT AI skill package into your project:
...
<Tip>
  ...
  ```bash
  npx skills add sthuthillc/kenectai --agent antigravity --global
  ```
...
<Note>
  For the best results, mention "KENECT AI" or "composition" in your prompt so the agent matches the right skills immediately instead of guessing at generic web-video conventions.
</Note>
...
<Steps>
  ...
  <Step title="Start the preview server">
    ...
    This launches the KENECT AI Studio in your browser with hot reload — edits to `index.html` appear instantly.
  </Step>
  ...
  <Step title="Prompt the agent">
    ...
    The agent reads the KENECT AI skill, writes valid HTML with `data-*` attributes and GSAP timelines, and saves it to your project files. The preview updates automatically.
  </Step>
...
Each agent picks up the relevant KENECT AI skills independently. Merge the output into a single `index.html` with `data-composition-src` references when all agents finish.

## MCP alternative

If you prefer zero-install cloud authoring, Antigravity supports remote MCP servers. Add the KENECT AI MCP connector to author and render compositions without the CLI — see [Antigravity's MCP documentation](https://antigravity.google/docs/mcp) for the exact setup steps in your version, then use the server URL:

```
https://mcp.heygen.com/mcp/hyperframes
```

Authorize via OAuth with your HeyGen account when prompted. See the [KENECT AI MCP guide](/guides/mcp) for full details on tools and prompting. The MCP handles rendering in the cloud; the CLI gives you local control.

## Agent instruction files

KENECT AI projects created with `npx @kenectai/cli init` scaffold both `CLAUDE.md` and `AGENTS.md` automatically — these give the agent additional composition context beyond what the skills provide.
...
- **Use `npx @kenectai/cli lint` before rendering.** The linter catches structural issues (missing `class="clip"`, unregistered timelines, muted video violations) that the agent might miss on complex edits.
- **Install registry blocks for advanced effects.** Run `npx @kenectai/cli add shimmer-sweep` to install pre-built blocks, then ask the agent to wire them into your composition.
- **Keep the preview server running.** Antigravity's file watcher + KENECT AI' hot reload means you see every agent edit in real time.
