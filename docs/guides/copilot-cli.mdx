...
description: "Set up KENECT AI with GitHub Copilot CLI — install skills, invoke them with slash commands, and render video from the terminal."
---

GitHub Copilot CLI brings AI-powered coding assistance to your terminal. KENECT AI skills teach it how to write correct compositions, GSAP timelines, and framework-specific patterns — so you get valid video HTML without reading the docs yourself.
...
- **Node.js 22+** and **FFmpeg** for the KENECT AI CLI
...
<Tip>
  ...
  ```bash
  npx skills add sthuthillc/kenectai --agent github-copilot --global
  ```
...
<Steps>
  ...
  <Step title="Start the preview server">
    ...
    Opens the KENECT AI Studio in your browser. Edits reload automatically.
  </Step>
  ...
  <Step title="Prompt with the skill">
    ...
    The agent writes valid KENECT AI HTML — `data-*` attributes, `class="clip"` on timed elements, paused GSAP timelines registered on `window.__timelines`. The preview updates as files are saved.
  </Step>
...
In agent mode, Copilot runs terminal commands (like `npx @kenectai/cli init` and `npx @kenectai/cli render`) on your behalf. Review the commands before approving them — especially if the skill pre-approves `shell` in its `allowed-tools`.

<Warning>
  Only pre-approve `shell` or `bash` in skill `allowed-tools` for skills you trust. The KENECT AI skills do not pre-approve shell access — the agent will ask for confirmation before running terminal commands.
</Warning>

## MCP alternative

Copilot CLI also supports MCP servers for cloud-based authoring without the local CLI. Use the `--additional-mcp-config` flag to add the KENECT AI MCP server to your session:
...
- **Use `/skills` to check skill status.** If output looks wrong, open the skills picker to verify the KENECT AI skills are enabled.
- **Run `npx @kenectai/cli lint` before rendering.** The linter catches structural issues the agent might miss on complex multi-scene edits.
...
- **Install registry blocks for transitions and effects.** `npx @kenectai/cli add shimmer-sweep` installs pre-built blocks, then prompt the agent to wire them in.
