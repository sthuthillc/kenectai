...
description: "How KENECT AI collects feedback, what data is collected, and how to opt out."
---

KENECT AI occasionally asks how a render went or how a Studio session felt. This page explains why we do it, when prompts appear, what data is collected, and how to disable them.
...
After a successful `kenectai render`, a short prompt may appear:
...
KENECT_FEEDBACK_INTERVAL=5 hyperframes render --output out.mp4
```
...
```
VITE_KENECT_FEEDBACK_INTERVAL=3
```

Invalid values (non-integer, zero, negative) fall back to the default.

**Note:** Disabling CLI telemetry (`kenectai telemetry disable`) does not suppress the Studio bar — Studio feedback is gated separately. The submitted data is still sent through the same anonymous PostHog pipeline, so the same privacy guarantees apply.

### `kenectai feedback` command
...
This command collects a doctor summary automatically, flushes telemetry, and exits. It appears under the **Settings** group in `kenectai --help`.
...
2. Publishes a minimal repro of the project and gets a public URL (the same upload as `kenectai publish`).
...
When an AI agent is detected, KENECT AI **skips the interactive readline prompt** and prints a structured hint instead:

```
  [hyperframes] Agent feedback: hyperframes feedback --rating <1-5> --comment "..."
```

Agents can then submit feedback using the `kenectai feedback` command above.
...
The `doctor_summary` is a compact string with environment context — included automatically so you don't need to run `kenectai doctor` when reporting a problem:
...
kenectai telemetry disable

# Or via environment variable (per-session)
KENECT_NO_TELEMETRY=1 kenectai render --output out.mp4

# Or via DO_NOT_TRACK (respects the global standard)
DO_NOT_TRACK=1 hyperframes render --output out.mp4
```

Once disabled, the `kenectai feedback` command will print `Telemetry is disabled. Feedback not sent.` and exit without sending anything.
...
kenectai render --quiet --output out.mp4
```
...
```
VITE_KENECT_FEEDBACK_INTERVAL=20
```

## Related

- [Telemetry](/packages/cli#telemetry) — full telemetry settings and what usage data is collected
- [`kenectai feedback`](/packages/cli#feedback) — CLI command reference
