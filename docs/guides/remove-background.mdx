...
Background removal — also called *matting* in VFX — separates a foreground subject (typically a person) from its background. The output is a video with an alpha channel: fully transparent where the background was, opaque where the subject is. Drop it into any KENECT AI composition as a `<video>` tag and the subject floats over whatever you put behind them.
...
<Steps>
  <Step title="Verify ffmpeg is installed">
    ...
    Confirm with `npx @kenectai/cli doctor` — both should be green.
  </Step>
  <Step title="Remove the background from your video">
    ```bash Terminal
    npx @kenectai/cli remove-background subject.mp4 -o transparent.webm
    ```
  ...
  <Step title="Drop it into a composition">
    ...
    Render the composition with the usual `kenectai render`.
  </Step>
</Steps>
...
```bash Terminal
npx @kenectai/cli remove-background subject.mp4 -o transparent.webm        # web playback
npx @kenectai/cli remove-background subject.mp4 -o transparent.mov         # editing
npx @kenectai/cli remove-background portrait.jpg -o cutout.png       # still image
```
...
```bash Terminal
npx @kenectai/cli remove-background subject.mp4 \
  -o subject.webm \
...
- **Opt into CUDA** by setting `HYPERFRAMES_CUDA=1` and providing a GPU-enabled `onnxruntime-node` build (the bundled build is CPU + CoreML only, to keep the install small for the 99% of users who don't have a GPU):
  ...
  ```bash Terminal
  HYPERFRAMES_CUDA=1 npx @kenectai/cli remove-background subject.mp4 -o transparent.webm --device cuda
  ```

Run `npx @kenectai/cli remove-background --info` to see what providers are detected on your machine and which one `auto` would pick.
...
**Subject over a KENECT AI scene:**
...
The CLI ships **one model on purpose** — the one that's MIT-licensed, runs everywhere, and produces production-quality output for person/portrait video. The list below leads with **free, open-source tools** that pair naturally with KENECT AI. Each entry calls out the actual catch — license, install effort, hardware needs — so you can pick the right one for your situation. Full benchmarks are in the [matting eval](https://www.heygenverse.com/a/0dd5a431-1832-4858-862d-de7fb7d02654).
...
After running any of these externally, encode the output as a KENECT AI-compatible transparent WebM with:
...
- **Person / portrait video, web playback, MIT-clean** → use the built-in `kenectai remove-background` (this is what it's tuned for).
...
The pipeline shells out to ffmpeg for decode + encode. Install via `brew install ffmpeg` on macOS or `sudo apt install ffmpeg` on Debian/Ubuntu. Verify with `npx @kenectai/cli doctor`.
...
- CLI: [`kenectai remove-background`](/packages/cli#remove-background)
