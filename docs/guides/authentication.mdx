...
<Steps>
  <Step title="Sign in">
    ...
    echo "$KENECT_API_KEY" | npx hyperframes auth login --api-key   # from stdin
    ```
  </Step>
...
3. `~/.kenectai/credentials` — written by `hyperframes auth login` (or `heygen auth login`)
