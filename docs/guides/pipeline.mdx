...
When the video is grounded in an existing source (a website, a brand site, a competitor reference), start with capture. KENECT AI ships a built-in capture command for websites:

```bash
npx @kenectai/cli capture https://example.com -o my-video/capture
```
...
`DESIGN.md` is also the input format for [Open Design](/guides/open-design) and [Claude Design](/guides/claude-design); both produce a `DESIGN.md` you can drop into a KENECT AI project.
...
| Techniques       | 2-3 picks from the [techniques library](https://github.com/sthuthillc/kenectai/blob/main/skills/hyperframes/references/techniques.md): SVG path drawing, Canvas 2D, CSS 3D, per-word typography, Lottie, video compositing, typing effects, variable fonts, MotionPath, velocity transitions, audio-reactive |
...
KENECT AI ships multiple TTS adapters (Kokoro, ElevenLabs, HeyGen); see [`/media-use`](/guides/prompting) for the skill that picks one. After generating audio, update `STORYBOARD.md` with the real beat boundaries from `transcript.json`.
...
```bash
npx @kenectai/cli lint                              # static HTML structure checks
npx @kenectai/cli check                             # one browser session: runtime errors, layout, motion, contrast
npx @kenectai/cli snapshot my-video --at 2.9,10.4   # PNGs at beat midpoints
```
...
The pipeline delivers the localhost Studio URL as the handoff. Your AI agent runs `npx @kenectai/cli preview` and shares the project URL. Rendering to MP4 is on-demand:

```bash
npx @kenectai/cli render --output my-video.mp4
```
...
- For surgical tweaks, open a composition file directly (e.g. `compositions/beat-3-proof.html`) and adjust animations, colors, or layout. `npx @kenectai/cli preview` shows changes live.
...
- Learning KENECT AI, because the artifacts leave every creative decision inspectable on disk.
