---
title: "@kenectai/aws-lambda"
description: "AWS Lambda and Step Functions adapter for distributed KENECT AI rendering."
---

The AWS Lambda package runs KENECT AI' distributed render primitives on AWS. It ships the Lambda handler, a Node client SDK, and an optional CDK construct for provisioning the render stack inside an adopter's AWS account.

```bash
npm install @kenectai/aws-lambda
```

## When to Use

**Use `@kenectai/aws-lambda` when you need to:**
...
| Import | Description |
|--------|-------------|
| `@kenectai/aws-lambda` | Handler types, runtime helpers, S3 transport, and client SDK exports |
| `@kenectai/aws-lambda/handler` | Lambda handler entry point for Step Functions dispatch |
| `@kenectai/aws-lambda/sdk` | Lightweight Node SDK for deploying sites, starting renders, polling progress, and estimating cost |
| `@kenectai/aws-lambda/cdk` | Optional CDK construct for provisioning the render bucket, Lambda, and Step Functions stack |

`aws-cdk-lib` and `constructs` are optional peer dependencies. SDK-only consumers do not need them unless they import from `@kenectai/aws-lambda/cdk`.
...
```typescript
import { deploySite, getRenderProgress, renderToLambda } from "@kenectai/aws-lambda/sdk";
