---
title: "@kenectai/engine"
description: "Seekable page-to-video capture engine using Chrome's BeginFrame API."
---

The engine package provides the low-level video capture pipeline: it loads an HTML page in headless Chrome, seeks to each frame independently, and captures pixel buffers using Chrome's `HeadlessExperimental.beginFrame` API. This is the layer that makes KENECT AI rendering deterministic.
...
<Warning>
  **Most users should NOT use the engine directly.** Use the [CLI](/packages/cli) (`npx @kenectai/cli render`) or the [producer](/packages/producer) package instead — they handle runtime injection, audio mixing, and encoding for you.
</Warning>

**Use `@kenectai/engine` when you need to:**
- Build a custom rendering pipeline with full control over frame capture
- Integrate KENECT AI capture into an existing video processing system
...
<Steps>
  ...
  <Step title="Load the composition">
    Your HTML composition is loaded into a browser page. The KENECT AI runtime is injected to manage timeline seeking.
  </Step>
...
```typescript
import { resolveConfig, DEFAULT_CONFIG } from '@kenectai/engine';
...
```typescript
import { spawnStreamingEncoder } from '@kenectai/engine';
...
```typescript
import { parseAudioElements, processCompositionAudio } from '@kenectai/engine';
...
```typescript
import { createFileServer } from '@kenectai/engine';
...
import type { HdrTransfer, HdrEncoderColorParams, HdrMasteringMetadata } from '@kenectai/engine';
...
The engine communicates with the browser page via the `window.__hf` protocol. Any page that implements this protocol can be captured by the engine — you are not limited to KENECT AI compositions.
...
The engine relies on the KENECT AI runtime's `window.__hf.seek(time)` function. When called, `seek`:
