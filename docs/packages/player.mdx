---
title: "@kenectai/player"
description: "Embeddable web component for playing KENECT AI compositions in any web page."
---

The player package provides a `<hyperframes-player>` custom element that embeds a KENECT AI composition anywhere — in any framework or plain HTML. Zero dependencies, 3KB gzipped.
...
**Use `@kenectai/player` when you need to:**
...
- Preview during development — use the [CLI](/packages/cli) (`npx @kenectai/cli preview`)
...
This is the canonical way to bridge the player into editor tools like [`@kenectai/studio`](/packages/studio). The studio exports a `resolveIframe` helper that handles both direct iframe refs and web-component refs:
...
The player communicates with the composition via the KENECT AI runtime bridge protocol (`postMessage`). Existing compositions work without modification.
