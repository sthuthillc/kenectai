---
title: "@kenectai/gcp-cloud-run"
description: "Google Cloud Run and Workflows adapter for distributed KENECT AI rendering."
---

The GCP Cloud Run package runs KENECT AI' distributed render primitives on Google Cloud. It ships a Cloud Run HTTP service, a Node client SDK, and a Terraform module for provisioning the bucket, service, workflow, and service accounts.

```bash
npm install @kenectai/gcp-cloud-run
```

## When to Use

**Use `@kenectai/gcp-cloud-run` when you need to:**
...
| Import | Description |
|--------|-------------|
| `@kenectai/gcp-cloud-run` | Server handler, event types, GCS transport, and client SDK exports |
| `@kenectai/gcp-cloud-run/server` | Cloud Run HTTP service entry point |
| `@kenectai/gcp-cloud-run/sdk` | Lightweight Node SDK for deploying sites, starting renders, polling progress, and estimating cost |
...
The service is intentionally close to the AWS Lambda adapter: thin cloud transport around the same `@kenectai/producer/distributed` primitives.
...
```typescript
import { getRenderProgress, renderToCloudRun } from "@kenectai/gcp-cloud-run/sdk";
