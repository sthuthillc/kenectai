---
title: "@kenectai/studio-server"
description: "The studio preview/editor backend — a mountable Hono API, extracted from core."
---

The studio-server package is the HTTP backend that powers studio preview and editing — project routes, file serving, preview bundling, thumbnails, and source-mutation helpers. It was extracted from `@kenectai/core/studio-api` into a dedicated package so an embedder can mount the studio backend **without** depending on core's full surface, and so core no longer ships a web server it doesn't need at render time.

```bash
npm install @kenectai/studio-server
```

## When to Use

<Tip>
  **Most users do not need this package directly.** The [CLI](/packages/cli) (`npx @kenectai/cli preview`) and [studio](/packages/studio) wire it up for you. Reach for it when you're **embedding** the studio backend into your own server.
</Tip>

**Use `@kenectai/studio-server` when you need to:**
...
<Info>
  `@kenectai/core/studio-api` still resolves (via a back-compat re-export stub), so existing imports keep working. New code should import from `@kenectai/studio-server` directly.
</Info>
...
| Import | Description |
|--------|-------------|
| `@kenectai/studio-server` | `createStudioApi`, helpers, types |
| `@kenectai/studio-server/source-mutation` | Source mutation utilities |
| `@kenectai/studio-server/screenshot-clip` | Element screenshot-clip geometry |
| `@kenectai/studio-server/manual-edits-render-script` | Manual-edits render body script |
| `@kenectai/studio-server/studio-motion-render-script` | Studio motion render body script |
| `@kenectai/studio-server/draft-markers` | Draft gesture-marker attributes |
| `@kenectai/studio-server/finite-mutation` | Finite-mutation safety checks |
...
```typescript
import { createStudioApi } from '@kenectai/studio-server';
import type { StudioApiAdapter, ResolvedProject } from '@kenectai/studio-server';
...
<CardGroup cols={2}>
  ...
  <Card title="CLI" icon="terminal" href="/packages/cli">
    `npx @kenectai/cli preview` wires this server up for you.
  </Card>
</CardGroup>
