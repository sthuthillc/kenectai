...
**Use `@kenectai/producer` when you need to:**
...
- Render from the command line without writing code — use the [CLI](/packages/cli) (`npx @kenectai/cli render`)
...
<Steps>
  ...
  <Step title="Inject the KENECT AI runtime">
...
```typescript
import { createRenderJob } from '@kenectai/producer';
...
```typescript
import type { ProgressCallback, RenderStatus } from '@kenectai/producer';
...
```typescript
import { RenderCancelledError } from '@kenectai/producer';
...
```typescript
import { startServer } from '@kenectai/producer/server';
...
```typescript
import { createRenderHandlers, createProducerApp } from '@kenectai/producer/server';
...
<Info>
  Docker mode requires Docker to be installed and running. Run `npx @kenectai/cli doctor` to verify your environment. See [Deterministic Rendering](/concepts/determinism) for details on what makes Docker mode deterministic.
</Info>
...
When GPU encoding is enabled, KENECT AI detects the available FFmpeg hardware encoder automatically. To check your system's capabilities:
...
```typescript
import { resolveConfig } from '@kenectai/producer';
