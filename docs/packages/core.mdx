...
The core package provides the foundational types, HTML parsing/generation, runtime, and composition linter that all other KENECT AI packages build on. If you are building tooling, writing a custom integration, or extending KENECT AI itself, this is the package you need.
...
<Tip>
  **Most users do not need to install `@kenectai/core` directly.** The [CLI](/packages/cli), [producer](/packages/producer), and [studio](/packages/studio) packages all depend on core internally. You only need it if you are doing one of the things listed below.
</Tip>

**Use `@kenectai/core` when you need to:**
...
- Access the KENECT AI type system for your own tooling
- Embed the KENECT AI runtime in a custom player

**Use a different package if you want to:**
- Preview compositions in the browser — use the [CLI](/packages/cli) (`npx @kenectai/cli preview`) or [studio](/packages/studio)
- Render compositions to MP4 — use the [CLI](/packages/cli) (`npx @kenectai/cli render`) or [producer](/packages/producer)
...
| Import | Description |
|--------|-------------|
| `@kenectai/core` | Types, parsers, generators, adapters, runtime utilities |
| `@kenectai/core/lint` | Composition linter |
| `@kenectai/core/compiler` | Timing compiler, HTML compiler, bundler, static guard |
| `@kenectai/core/runtime` | Pre-built IIFE runtime for browser injection |
...
import { getDefaultStageZoom } from '@kenectai/core';
```
...
import type { ParsedHtml, CompositionMetadata } from '@kenectai/core';
...
import { extractCompositionMetadata } from '@kenectai/core';
...
import { getVariables } from '@kenectai/core';
const { title } = getVariables<{ title: string }>();

// Validate CLI / host overrides against the declared schema:
import { validateVariables, formatVariableValidationIssue } from '@kenectai/core';
...
<Info>
  The GSAP + HTML parsing layer now lives in its own standalone package, [`@kenectai/parsers`](/packages/parsers). Core re-exports the API below for back-compat; import from `@kenectai/parsers` directly in new code.
</Info>
...
// GSAP parsing, mutation, and constants live in @kenectai/parsers:
import { parseGsapScript, SUPPORTED_PROPS, SUPPORTED_EASES } from '@kenectai/parsers/gsap-parser';
import { updateAnimationInScript, addAnimationToScript, removeAnimationFromScript } from '@kenectai/parsers/gsap-writer-acorn';
import type { GsapAnimation, GsapMethod, ParsedGsap } from '@kenectai/core';
...
<Info>
  The composition linter now lives in its own package, [`@kenectai/lint`](/packages/lint) — install it directly to lint a project or HTML string from Node without the CLI. `@kenectai/core/lint` remains a back-compat re-export.
</Info>

The composition linter checks for structural issues that would cause rendering failures or unexpected behavior. You can run it from the CLI with `npx @kenectai/cli lint`, or call it programmatically:

```typescript
import { lintHyperframeHtml, lintMediaUrls } from '@kenectai/core/lint';
import type {
  HyperframeLintResult,
  HyperframeLintFinding,
  HyperframeLintSeverity,     // "error" | "warning" | "info"
  HyperframeLinterOptions,
} from '@kenectai/core/lint';
...
import { compileHtml } from '@kenectai/core/compiler';
import type { MediaDurationProber } from '@kenectai/core/compiler';
...
import { bundleToSingleHtml } from '@kenectai/core/compiler';
import type { BundleOptions } from '@kenectai/core/compiler';
...
import { validateHyperframeHtmlContract } from '@kenectai/core/compiler';
import type {
...
The KENECT AI runtime manages playback, seeking, and clip lifecycle in the browser. The core package provides utilities for building and loading the runtime:
...
```typescript
import runtime from '@kenectai/core/runtime';
```
...
import type { MediaVisualStyleProperty } from '@kenectai/core';
...
<CardGroup cols={2}>
  <Card title="@kenectai/parsers" icon="code" href="/packages/parsers">
