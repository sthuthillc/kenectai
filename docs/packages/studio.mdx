---
title: "@kenectai/studio"
description: "Visual composition editor with live preview, timeline view, and hot reload."
---

The studio package provides a browser-based visual editor for creating and previewing KENECT AI compositions. It gives you a real-time preview of your video, a visual timeline of all clips, and player controls for seeking and playback — all updating live as you edit your HTML.
...
**Use `@kenectai/studio` when you need to:**
- Build a custom composition editor UI (e.g., embedded in your own web application)
- Integrate the KENECT AI preview player into a larger product
- Extend the editor with custom panels, toolbars, or integrations

**Use a different package if you want to:**
- Preview compositions during development — use the [CLI](/packages/cli) (`npx @kenectai/cli preview`), which launches the studio for you
...
<Tip>
  **For most development workflows, you do not need to install the studio directly.** Running `npx @kenectai/cli preview` starts the studio automatically with hot reload. Install `@kenectai/studio` only if you are embedding the editor into your own application.
</Tip>
...
bun run --filter @kenectai/studio dev
```
...
| Import | Description |
|--------|-------------|
| `@kenectai/studio` | React components, hooks, and types |
| `@kenectai/studio/tailwind-preset` | Tailwind CSS preset for studio styling |
...
```typescript
import { NLELayout, NLEPreview, CompositionBreadcrumb } from '@kenectai/studio';
import type { CompositionLevel } from '@kenectai/studio';
...
import {
  ...
  PlayerControls,
  Timeline,
} from '@kenectai/studio';
import type { TimelineElement } from '@kenectai/studio';
...
```typescript
import { SourceEditor, PropertyPanel, FileTree } from '@kenectai/studio';
...
```typescript
import { StudioApp } from '@kenectai/studio';
...
```typescript
import { useTimelinePlayer } from '@kenectai/studio';
...
```typescript
import { usePlayerStore, liveTime, formatTime } from '@kenectai/studio';
...
```typescript
import { useElementPicker } from '@kenectai/studio';
...
The studio renders your composition in an iframe using the KENECT AI runtime. What you see in the preview is exactly what will be captured during rendering — the same runtime code, the same seek logic, the same clip lifecycle.
...
1. **Iframe preview** — your composition HTML is loaded in an isolated iframe with the KENECT AI runtime injected. This ensures the preview uses the same rendering path as production.
...
3. **Timeline component** — parses the composition using `@kenectai/core` to extract clip timing data and renders the visual timeline panel.
...
```typescript
// tailwind.config.ts
import studioPreset from '@kenectai/studio/tailwind-preset';
...
<CardGroup cols={2}>
  <Card title="CLI" icon="terminal" href="/packages/cli">
    Launches the studio via `npx @kenectai/cli preview` — the easiest way to preview compositions.
  </Card>
