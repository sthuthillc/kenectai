...
<Tabs>
  <Tab title="Create">
    ...
    `add` reads [`hyperframes.json`](#hyperframes-json) at the project root to know which registry to pull from and where to drop files. If the file is missing but the directory looks like a KENECT AI project (has `index.html`), a default `hyperframes.json` is written the first time you run `add`.
...
`~/.kenectai/credentials` (mode `0600`) and are **shared with the
...
echo "$KENECT_API_KEY" | hyperframes auth login --api-key
```
...
Render a KENECT AI composition on HeyGen's hosted cloud — no local Chrome, no local ffmpeg, no AWS to manage. Sign in once with `kenectai auth login` and the same credential drives every `cloud` subcommand.
...
- `kenectai cloud render`: zero-infra option. HeyGen runs the render; you pay per credit. Use when you don't want to manage Chrome/ffmpeg/AWS locally.
...
`kenectai init` writes a `hyperframes.json` file at the root of every new project. `kenectai add` reads it to know which registry to pull items from and where to drop them. Edit the file (or delete it to fall back to defaults) to reshape your project layout or point at a custom registry.

```json
{
  "$schema": "https://docs.kenectai.com/schema/hyperframes.json",
