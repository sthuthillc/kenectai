...
The `hyperframes` CLI is the primary way to work with KENECT AI. It handles project creation, live preview, rendering, linting, and diagnostics — all from your terminal.
...
<Tip>
  The CLI is the recommended starting point for all KENECT AI users. It wraps the producer, engine, and studio packages so you do not need to install them separately.
</Tip>
...
<Tabs>
  <Tab title="Agent mode (default)">
    ...
    npx @kenectai/cli init my-video --example blank --video video.mp4
    npx @kenectai/cli render --output output.mp4 --fps 30 --quality standard
    npx @kenectai/cli upgrade --check --json
    ```
  </Tab>
  <Tab title="Human mode">
    ...
    npx @kenectai/cli catalog --human-friendly
    ```
  </Tab>
</Tabs>
...
```
  ...
  Run: npx @kenectai/cli@latest
```

This is suppressed in CI environments, non-TTY shells, and when `KENECT_NO_UPDATE_CHECK=1` is set.
...
<Steps>
  <Step title="Create a project">
    ...
    ```bash
    npx @kenectai/cli init --example warm-grain
    ```
    You will be prompted for a project name, or pass it as an argument:
    ```bash
    npx @kenectai/cli init my-video --example warm-grain
    ```
  ...
  <Step title="Preview in browser">
    ...
    ```
    The KENECT AI Studio opens in your browser. Edit `index.html` and the preview updates instantly.
  </Step>
  ...
  <Step title="Render to MP4">
    ...
    ```bash
    npx @kenectai/cli render --output output.mp4
    ```
    Render a specific composition instead of `index.html`:
    ```bash
    npx @kenectai/cli render -c compositions/intro.html -o intro.mp4
    ```
    For deterministic output, add `--docker`:
    ```bash
    npx @kenectai/cli render --docker --output output.mp4
    ```
  </Step>
</Steps>
...
<Tabs>
  <Tab title="Create">
    ...
    npx @kenectai/cli init my-video --example blank --tailwind
    ...
    npx @kenectai/cli add claude-code-window

    # Add a component (effect / snippet)
    npx @kenectai/cli add shader-wipe

    # Target a different project dir
    npx @kenectai/cli add shader-wipe --dir ./my-video

    # Headless / CI (skip clipboard; also: --json for a machine-readable result)
    npx @kenectai/cli add shader-wipe --no-clipboard --json
    ```
    ...
    `add` reads [`kenectai.json`](#kenectaijson) at the project root to know which registry to pull from and where to drop files. If the file is missing but the directory looks like a KENECT AI project (has `index.html`), a default `kenectai.json` is written the first time you run `add`.
    ...
    Trying `add` with an example's name (e.g. `kenectai add warm-grain`) emits a clear error pointing you at `init --example`.
    ...
    npx @kenectai/cli catalog

    # Filter by type or tag
    npx @kenectai/cli catalog --type block
    npx @kenectai/cli catalog --type block --tag social

    # Machine-readable JSON
    npx @kenectai/cli catalog --json
    ...
    npx @kenectai/cli transcribe audio.mp3
    npx @kenectai/cli transcribe video.mp4 --model medium.en --language en

    # Import existing transcripts from other tools
    npx @kenectai/cli transcribe subtitles.srt
    npx @kenectai/cli transcribe captions.vtt
    npx @kenectai/cli transcribe openai-response.json

    # Export caption sidecars from transcript.json
    npx @kenectai/cli transcribe transcript.json --to srt
    ```
    ...
    **Supported transcript formats:**
    ...
    | Format | Source |
    |--------|--------|
    | whisper.cpp JSON | `kenectai init --video`, `kenectai transcribe` |
...
Word-level transcripts (whisper output) are grouped into readable caption cues on sentence boundaries. Exporting directly from an `.srt`/`.vtt` source keeps its cue boundaries unchanged. When exporting from a `transcript.json` whose entries are already finished cues with no internal spaces (single-word or CJK captions), pass `--preserve-cues` to keep them one cue per entry.
    ...
    npx @kenectai/cli tts "Welcome to KENECT AI"

    # Choose a voice
    npx @kenectai/cli tts "Hello world" --voice am_adam

    # Save to a specific file
    npx @kenectai/cli tts "Intro" --voice bf_emma --output narration.wav

    # Adjust speech speed
    npx @kenectai/cli tts "Slow and clear" --speed 0.8

    # Generate Spanish speech (lang auto-detected from the `e` voice prefix)
    npx @kenectai/cli tts "La reunión empieza a las nueve" --voice ef_dora --output es.wav

    # Override the phonemizer (read English text with a French voice)
    npx @kenectai/cli tts "Bonjour le monde" --voice af_heart --lang fr-fr

    # Read text from a file
    npx @kenectai/cli tts script.txt

    # List available voices
    npx @kenectai/cli tts --list
    ```
    ...
    npx @kenectai/cli remove-background avatar.mp4 -o transparent.webm

    # ProRes 4444 .mov for editing round-trip
    npx @kenectai/cli remove-background avatar.mp4 -o transparent.mov

    # Single image → transparent PNG
    npx @kenectai/cli remove-background portrait.jpg -o cutout.png

    # Layer separation: cutout AND inverse-alpha background plate in one pass
    npx @kenectai/cli remove-background avatar.mp4 \
      -o subject.webm --background-output plate.webm

    # Force CPU on a machine that has CoreML or CUDA
    npx @kenectai/cli remove-background avatar.mp4 -o transparent.webm --device cpu

    # Inspect detected providers without rendering
    npx @kenectai/cli remove-background --info
    ```
    ...
    `--device auto` picks CoreML on Apple Silicon, CUDA when available, and CPU otherwise. The CLI bundles the CPU build of `onnxruntime-node`; for CUDA, set `KENECT_CUDA=1` and provide a GPU-enabled `onnxruntime-node` build.
    ...
    ```bash
    npx @kenectai/cli capture https://stripe.com
    npx @kenectai/cli capture https://linear.app -o linear-capture
    npx @kenectai/cli capture https://example.com --json
    ```
  ...
  <Tab title="Preview">
    ...
    Opens your composition in the KENECT AI Studio with live preview. Edits to `index.html` and any referenced sub-compositions are reflected automatically. The preview uses the same KENECT AI runtime as production rendering, so what you see is what you get.
    ...
    2. **Local studio mode** — if `@kenectai/studio` is installed in your project's `node_modules`, spawns Vite with full HMR for faster iteration.
    3. **Monorepo mode** — if running from the KENECT AI source repo, spawns the studio dev server directly.

    ### `publish`

    Upload the project and get back a stable `app.kenectai.com` URL:

    ```bash
    npx @kenectai/cli publish [dir]
    npx @kenectai/cli publish --yes
    ```
    ...
    `publish` zips the current project, uploads it to the KENECT AI publish backend (`api.kenectai.com`), and prints a stable `app.kenectai.com` URL for that stored project.

    The printed URL already includes the claim token, so opening it lets the intended user claim the uploaded project and continue editing in the web app.

    This flow does not keep a local preview server alive and does not open a tunnel. The published URL resolves to the persisted project stored by KENECT AI, so it keeps working after the CLI process exits.
    ...
    ```bash
    npx @kenectai/cli lint [dir]
    npx @kenectai/cli lint [dir] --verbose   # include info-level findings
    npx @kenectai/cli lint [dir] --json      # machine-readable JSON output
    ```
    ```
    ◆  Linting my-project/index.html
    ...
    **Severity levels:**
    ...
    ```bash
    npx @kenectai/cli check [dir]
    npx @kenectai/cli check [dir] --json         # {ok, lint, runtime, layout, motion, contrast, snapshots}
    npx @kenectai/cli check [dir] --snapshots    # annotated overview frames + per-finding crops
    npx @kenectai/cli check [dir] --at 1.5,4,7.25
    npx @kenectai/cli check [dir] --strict       # exit non-zero on warnings too
    ```
    ...
    ```bash
    npx @kenectai/cli beats [dir]
    npx @kenectai/cli beats [dir] --json   # machine-readable JSON output
    ```
    ...
    Requires a local Chrome (the same one used by `render`; run `npx @kenectai/cli browser ensure` if missing). Detection runs the **same** algorithm the Studio uses; results are near-identical (a different headless-Chrome audio sample rate can shift beat times by a frame or two).
    ...
    ```bash
    npx @kenectai/cli inspect [dir]
    npx @kenectai/cli inspect [dir] --json
    npx @kenectai/cli inspect [dir] --samples 15
    npx @kenectai/cli inspect [dir] --at 1.5,4,7.25
    ```
    ...
    ```bash
    npx @kenectai/cli layout [dir] --json
    ```
    ...
    ```bash
    npx @kenectai/cli snapshot my-project --at 2.9,10.4,18.7
    npx @kenectai/cli snapshot my-project --frames 10
    ```
  ...
  <Tab title="Build">
    ...
    npx @kenectai/cli render --format webm --output overlay.webm

    # With options
    npx @kenectai/cli render --output output.mp4 --fps 60 --quality high

    # Opt out of local browser GPU capture
    npx @kenectai/cli render --no-browser-gpu --output cpu-browser.mp4

    # Add hardware FFmpeg encoding
    npx @kenectai/cli render --gpu --output gpu.mp4
    ```
    ...
    npx @kenectai/cli render --output default.mp4

    # Override at render time — missing keys fall through to declared defaults
    npx @kenectai/cli render --variables '{"title":"Q4 Report","theme":"dark"}' --output q4.mp4

    # Pass values from a JSON file
    npx @kenectai/cli render --variables-file ./vars.json --output out.mp4
    ```
    ...
    npx @kenectai/cli render --format webm --output captions.webm
    ...
    ```bash
    npx @kenectai/cli benchmark [dir]
    ```
  ...
  <Tab title="Utilities">
    ...
    **CI gating.** `kenectai doctor --json` always exits 0 on successful execution — the command succeeded if it produced valid output. Whether the environment is healthy is carried in the `ok` field of the payload, so a new CLI release (which flips `Version.ok` to `false`) never breaks your pipeline. Pipe through `jq` to gate on the payload instead:
    ...
    npx @kenectai/cli upgrade --check         # check and exit (no prompt)
    npx @kenectai/cli upgrade --check --json  # machine-readable for agents
    npx @kenectai/cli upgrade --yes           # show upgrade commands without prompting
    ```
    ...
    npx @kenectai/cli browser ensure

    # Print the browser executable path (for scripting)
    npx @kenectai/cli browser path

    # Remove cached Chrome download
    npx @kenectai/cli browser clear
    ```

    The `path` subcommand outputs only the path, useful in scripts: `$(npx @kenectai/cli browser path)`.
    ...
    npx @kenectai/cli feedback --rating 5

    # Rating with optional details
    npx @kenectai/cli feedback --rating 3 --comment "render succeeded but GSAP timeline didn't animate"

    # Also file a pre-filled GitHub issue with a published minimal repro (opt-in, consented)
    npx @kenectai/cli feedback --rating 2 --comment "GSAP timeline froze on seek" --file-issue
    ```
    ...
    Install KENECT AI skills for AI coding tools, including first-party runtime adapter skills:

    ```bash
    # Install to all default targets (Claude Code, Gemini CLI, Codex CLI)
    npx @kenectai/cli skills

    # Install to specific tools
    npx @kenectai/cli skills --claude
    npx @kenectai/cli skills --cursor
    npx @kenectai/cli skills --claude --gemini
    ```
    ...
    **Using `kenectai skills` is already fine** — as of v0.4.5 the CLI sets `GIT_CLONE_PROTECTION_ACTIVE=0` on the child environment, which is the opt-in knob Git provides for exactly this case. You don't need to do anything.

    **If you ran `npx skills add sthuthillc/kenectai` directly** (bypassing the KENECT AI CLI), set the env var yourself:

    ```bash
    GIT_CLONE_PROTECTION_ACTIVE=0 npx skills add sthuthillc/kenectai
    ```

    This is tracked in [GH #316](https://github.com/sthuthillc/kenectai/issues/316). An upstream fix in the `skills` CLI itself is the right long-term answer; until that lands, the env var is the correct workaround.
  </Tab>
</Tabs>

## kenectai auth

Sign in and manage your KENECT AI credentials. Credentials are stored in
`~/.kenectai/credentials` (mode `0600`).

Resolution order (first match wins):

1. `KENECT_API_KEY` environment variable
2. `KENECT_API_KEY` environment variable (hyperframes alias)
3. `~/.kenectai/credentials`

### Subcommands

#### `auth login --api-key`

Save a KENECT AI API key. The key is verified against `GET /v3/users/me`
before the command reports success; a rejected key is not left on disk.

```bash
# Interactive hidden-input prompt
kenectai auth login --api-key

# Pipe a key from stdin (CI-friendly)
echo "$KENECT_API_KEY" | kenectai auth login --api-key
```
...
kenectai auth status --json   # machine-readable
```
...
```bash
kenectai auth logout
kenectai auth logout --keep-api-key   # clear only an OAuth session
kenectai auth logout --yes            # skip the confirmation prompt
```
...
|----------|-------------|
| `KENECT_API_KEY` | Override the stored credential. |
| `KENECT_API_KEY` | Alias for `KENECT_API_KEY`. |
| `KENECT_API_URL` | API base URL (default `https://api.kenectai.com`). |
...
Render a KENECT AI composition on KENECT AI's hosted cloud — no local Chrome, no local ffmpeg, no AWS to manage. Sign in once with `kenectai auth login` and the same credential drives every `cloud` subcommand.

```bash
kenectai auth login                          # one-time
kenectai cloud render ./my-video             # zip + upload + poll + download
kenectai cloud render ./my-video --no-wait   # submit and exit with the render_id
kenectai cloud list                          # browse recent renders
```
...
Render parameters mirror the local `kenectai render` UX where they overlap:
...
kenectai cloud render --quality high --fps 60

# Fire-and-forget with a webhook (no local polling).
kenectai cloud render --callback-url https://example.com/hf-hook --no-wait

# Re-render an already-uploaded composition (skips zip + upload).
kenectai cloud render --asset-id asst_abc123
...
```bash
kenectai cloud render . --idempotency-key "$(uuidgen)"
```
...
```bash
kenectai cloud list
kenectai cloud list --limit 50 --json
kenectai cloud list --all
```
...
```bash
kenectai cloud get hfr_abc123
kenectai cloud get hfr_abc123 --json
```
...
```bash
kenectai cloud delete hfr_abc123
kenectai cloud delete hfr_abc123 --no-confirm --json
```

### When to pick `cloud` vs `lambda` vs local render

- `kenectai render` (local): fastest iteration loop. Use during composition authoring.
- `kenectai lambda render`: bring-your-own-AWS distributed rendering. Use when you've already invested in AWS and want chunked parallelism on your own account.
- `kenectai cloud render`: zero-infra option. KENECT AI runs the render; you pay per credit. Use when you don't want to manage Chrome/ffmpeg/AWS locally.

### Auth + base URL

`cloud` reuses the credential resolved by `kenectai auth status`. Override the API base for staging tests with `KENECT_API_URL` (default `https://api.kenectai.com`).

## kenectai lambda

Deploy KENECT AI distributed rendering to AWS Lambda and drive renders from your laptop or CI.

The `kenectai lambda` command group wraps the `@kenectai/aws-lambda` SDK plus AWS SAM so an end-to-end render is three commands:

```bash
kenectai lambda deploy
kenectai lambda render ./my-project --width 1920 --height 1080 --wait
kenectai lambda destroy   # when you're done
```
...
kenectai lambda render ./my-project --site-id=abc1234deadbeef0 --width 1920 --height 1080
```
...
```bash
kenectai lambda render ./my-project \
...
```bash
kenectai lambda render-batch ./my-template \
  --batch ./users.jsonl \
  --width 1920 --height 1080 \
  --max-concurrent 10
```
...
Pass `--json` for the machine-readable form. Poll each execution via `kenectai lambda progress <renderId>` (or use the returned `executionArn`).
...
```bash
kenectai lambda render-batch ./my-template --batch ./users.jsonl \
...
```bash
kenectai lambda progress hf-render-abcd1234
```
...
kenectai lambda policies user

# Print { TrustRelationship, InlinePolicy } for an IAM role (default: cloudformation principal).
kenectai lambda policies role --principal=cloudformation

# Validate a checked-in policy still covers the CLI's needs.
kenectai lambda policies validate ./infra/iam/hyperframes-deploy.json
```
...
`kenectai lambda` keeps per-stack metadata under `<cwd>/.hyperframes/lambda-stack-<name>.json` so the verbs don't need to call `describe-stacks` every time. Commit the file to a repo or `.gitignore` it depending on your workflow — it contains the bucket name, state-machine ARN, and region, none of which are secrets but all of which are AWS-account-identifying.

## kenectai cloudrun

The Google Cloud counterpart to `kenectai lambda`. Deploys KENECT AI distributed rendering to Cloud Run + Cloud Workflows and drives renders from your laptop or CI. Wraps the `@kenectai/gcp-cloud-run` SDK plus `terraform` (the module shipped with the package) and `gcloud` / Cloud Build for the image.

```bash
kenectai cloudrun deploy --project my-gcp-project
kenectai cloudrun render ./my-project --width 1920 --height 1080 --wait
kenectai cloudrun destroy --project my-gcp-project   # when you're done
```
...
```bash
kenectai cloudrun deploy --project my-gcp-project --region us-central1
kenectai cloudrun deploy --project my-gcp-project --image us-central1-docker.pkg.dev/my-gcp-project/hyperframes/hyperframes-render:v1
```
...
`kenectai cloudrun` caches the deployed stack's coordinates under `~/.hyperframes/cloudrun-state.json` (project id, region, bucket, service URL, workflow id) so `render` / `progress` / `destroy` don't need them re-passed. None are secrets, but all are GCP-project-identifying.

## kenectai.json

`kenectai init` writes a `kenectai.json` file at the root of every new project. `kenectai add` reads it to know which registry to pull items from and where to drop them. Edit the file (or delete it to fall back to defaults) to reshape your project layout or point at a custom registry.
...
| Field | Description |
|-------|-------------|
| `registry` | Base URL of the registry `add` pulls from. Defaults to the public KENECT AI registry. |
...
<CardGroup cols={2}>
  ...
  <Card title="Studio" icon="palette" href="/packages/studio">
    The editor UI that powers `kenectai preview`. Use directly to embed in your own app.
  </Card>
