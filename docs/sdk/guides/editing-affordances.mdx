...
<Note>
  This API lives on the `@kenectai/sdk/editing` subpath, available since `@kenectai/sdk@0.7.22`. If the import fails to resolve, upgrade: `npm install @kenectai/sdk@latest`.
</Note>
...
**Browser-only.** `resolveElementAffordances` calls `getComputedStyle` internally. It must not be imported or called in Node, a server action, or any non-browser path. The SDK adapter (`@kenectai/sdk/editing`) is separate from the pure core resolver precisely to keep the Node SDK path free of browser globals.
...
The pure DOM-free resolver (`resolveEditingAffordances`, `EditableElementFacts`) lives in `@kenectai/core` and is documented at [`/packages/core`](/packages/core).
