...
description: "History module, persist queue, document utilities, constants, and error types exported from @kenectai/sdk."
---

This page covers everything exported from `@kenectai/sdk` that is not covered in [`openComposition`](/sdk/reference/open-composition), the [`Composition` interface](/sdk/reference/composition), [edit operations](/sdk/reference/edit-operations), [types](/sdk/reference/types), or [adapters](/sdk/reference/adapters).
...
```typescript
import { createHistory } from "@kenectai/sdk";
import type { HistoryModule, HistoryOptions, HistoryEntry } from "@kenectai/sdk";
```
...
```typescript
import { openComposition, createHistory } from "@kenectai/sdk";
...
```typescript
import { createPersistQueue } from "@kenectai/sdk";
import type { PersistQueueModule, PersistQueueOptions } from "@kenectai/sdk";
```
...
```typescript
import { openComposition, createPersistQueue } from "@kenectai/sdk";
...
```typescript
import { buildDocument, buildRoots, flatElements } from "@kenectai/sdk";
```
...
```typescript
import { buildDocument, flatElements } from "@kenectai/sdk";
...
```typescript
import { resolveScoped, findById, bareId, escapeHfId, isNewHostBoundary } from "@kenectai/sdk";
```
...
```typescript
import { readVariableDefault } from "@kenectai/sdk";
```
...
```typescript
import { ORIGIN_APPLY_PATCHES } from "@kenectai/sdk";

const ORIGIN_APPLY_PATCHES = "@kenectai/sdk:applyPatches";
```
...
The value is a namespaced string rather than a `Symbol` so it survives realm boundaries — `postMessage`, structured clone, and JSON serialization all preserve it correctly. T3 embedded hosts that forward patch events across frames or workers rely on this property. The namespace prefix (`@kenectai/sdk:`) makes accidental collision with a host-chosen origin string negligible.

### ORIGIN\_LOCAL

```typescript
import { ORIGIN_LOCAL } from "@kenectai/sdk";
...
```typescript
import { UnsupportedOpError } from "@kenectai/sdk";
```
...
```typescript
import { resolveNearestHfElement } from "@kenectai/sdk";
...
```typescript
import { resolveElementAffordances } from "@kenectai/sdk/editing";
```

Determines which editing operations are available for a live element given its current DOM state and model. Imported from the `@kenectai/sdk/editing` subpath. Full documentation is in the [Editing Affordances guide](/sdk/guides/editing-affordances).
