...
```typescript
import type { PersistAdapter } from "@kenectai/sdk";
```
...
```typescript
import type { PreviewAdapter } from "@kenectai/sdk";
```
...
<Note>
  `ElementAtPointResult` and `DraftProps` are the structural shapes a `PreviewAdapter` produces and consumes. They are **not** re-exported from the `@kenectai/sdk` barrel — you implement against these shapes rather than importing them.
</Note>
...
```typescript
import { createMemoryAdapter } from "@kenectai/sdk";
...
**Node.js only.** Returns a `PersistAdapter` that reads and writes files under a root directory. Import from the `@kenectai/sdk/adapters/fs` subpath — this module uses Node `fs/promises` and is excluded from the browser-safe main bundle.
...
<Warning>
  `createFsAdapter` uses Node.js `fs/promises`. Do not import it in browser or edge environments — import from `@kenectai/sdk/adapters/fs` (the subpath) so bundlers can tree-shake it.
</Warning>
...
```typescript
import { createHeadlessAdapter } from "@kenectai/sdk";
...
```typescript
import { openComposition, createHeadlessAdapter } from "@kenectai/sdk";
...
```typescript
import { createIframePreviewAdapter } from "@kenectai/sdk";
...
```typescript
import { openComposition, createIframePreviewAdapter } from "@kenectai/sdk";
...
|--------|---------------|
| `PersistAdapter`, `PreviewAdapter`, `PersistVersionEntry` | `@kenectai/sdk` (types only) |
| `createMemoryAdapter` | `@kenectai/sdk` |
| `createHeadlessAdapter` | `@kenectai/sdk` |
| `createIframePreviewAdapter`, `resolveNearestHfElement` | `@kenectai/sdk` |
| `createFsAdapter`, `FsAdapterOptions` | `@kenectai/sdk/adapters/fs` |
