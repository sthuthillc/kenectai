---
title: Contributing
description: "How to contribute to KENECT AI."
---

Thanks for your interest in contributing to KENECT AI! This guide covers everything you need to get set up, run tests, and submit a pull request.
...
<Steps>
  ...
  <Step title="Install dependencies">
    KENECT AI uses [bun](https://bun.sh/) for package management:
    ```bash
    bun install
    ```
  </Step>
...
<CodeGroup>
```bash Core
bun run --filter @kenectai/core test
```
```bash Engine
bun run --filter @kenectai/engine test
```
```bash Runtime Contract
bun run --filter @kenectai/core test:hyperframe-runtime-ci
```
...
|---------|------|-------------|
| [`@kenectai/core`](/packages/core) | `packages/core` | Types, HTML generation, runtime, linter |
| [`@kenectai/sdk`](/packages/sdk) | `packages/sdk` | Headless composition editing engine |
| [`@kenectai/engine`](/packages/engine) | `packages/engine` | Seekable page-to-video capture engine |
| [`@kenectai/player`](/packages/player) | `packages/player` | Embeddable composition player |
| [`@kenectai/producer`](/packages/producer) | `packages/producer` | Full rendering pipeline (capture + encode) |
| [`@kenectai/shader-transitions`](/packages/shader-transitions) | `packages/shader-transitions` | WebGL shader transition engine |
| [`@kenectai/aws-lambda`](/packages/aws-lambda) | `packages/aws-lambda` | AWS Lambda distributed rendering adapter |
| [`@kenectai/gcp-cloud-run`](/packages/gcp-cloud-run) | `packages/gcp-cloud-run` | GCP Cloud Run distributed rendering adapter |
| [`@kenectai/studio`](/packages/studio) | `packages/studio` | Composition editor UI |
| [`hyperframes`](/packages/cli) | `packages/cli` | CLI for creating, previewing, and rendering |
| `@kenectai/sdk-playground` (private) | `packages/sdk-playground` | Local SDK playground app |
...
- Use [GitHub Issues](https://github.com/sthuthillc/kenectai/issues) for bug reports and feature requests
- Search existing issues before creating a new one
- For bug reports, include:
  - Steps to reproduce
  - Expected behavior vs. actual behavior
  - KENECT AI version (`npx @kenectai/cli info`)
  - Operating system and Node.js version

## Community

<CardGroup cols={2}>
  <Card title="GitHub Issues" icon="github" href="https://github.com/sthuthillc/kenectai/issues">
    Report bugs, request features, and discuss ideas.
  </Card>
  <Card title="Code of Conduct" icon="handshake" href="https://github.com/sthuthillc/kenectai/blob/main/CODE_OF_CONDUCT.md">
...
By contributing, you agree that your contributions will be licensed under the [Apache 2.0 License](https://github.com/sthuthillc/kenectai/blob/main/LICENSE).
