---
title: "Changelog"
description: "Release notes for KENECT AI."
rss: true
---

Recent KENECT AI releases, including user-facing features, fixes, and migration notes.

{/* New release entries are prepended by `bun run changelog:draft <version> --write`. */}

<Update
  label="KENECT AI v0.7.57"
...
[View the full commit range](https://github.com/sthuthillc/kenectai/compare/v0.7.56...v0.7.57).
</Update>

<Update
  label="KENECT AI v0.7.56"
  description="Released - 2026-07-14"
  tags={["Release", "CLI", "Docs"]}
>
...
[View the full commit range](https://github.com/sthuthillc/kenectai/compare/v0.7.55...v0.7.56).
</Update>

<Update
  label="KENECT AI v0.7.55"
  description="Released - 2026-07-14"
  tags={["Release", "Producer", "CLI", "Media"]}
>
Completes the KENECT AI rebrand of the agent skills and internal build tooling, adds a native Gemini TTS provider and rewrites the Lyria background-music generator onto Google's current batch API, and carries forward upstream parallel-streaming and rendering fixes.

## KENECT AI

- **Skills:** Renamed all 8 `hyperframes-*` skill directories to `kenectai-*` (e.g. `/kenectai-core`, `/kenectai-animation`) and fixed the CLI's default skill-install source, core-skill detection, and a CI env-var mismatch that had silently broken the skip-skills opt-out.
- **Media:** Added a Gemini native TTS provider (`gemini-3.1-flash-tts-preview`) to the audio engine's provider chain, and rewrote the Lyria background-music recipe off the old live-streaming session API onto the batch `lyria-3-pro-preview` Interactions API — dropping the `google-genai` pip dependency entirely.
- **Build:** Renamed the internal `hyperframe-runtime` build/test npm scripts and script files to `kenectai-runtime`, and fixed 12 packages' stale `repository.url` still pointing at the upstream HeyGen repo.
...
[View the full commit range](https://github.com/heygen-com/hyperframes/compare/v0.7.53...v0.7.55).
</Update>
...
>
Adds live-DOM sync and variable CRUD to `@kenectai/sdk`, plus mechanical Figma motion translation with an objective fidelity gate. Also ships several render-pipeline and parser fixes (cross-realm position edits, static-frame dedup call-detection, GSAP position holds).
...
- **Sdk:** Pin editing-affordances availability to @kenectai/sdk@0.7.22 ([9140ee51b](https://github.com/heygen-com/hyperframes/commit/9140ee51b3109b97346cda083714e0b13b1270cf))
...
>
This release fixes the `@kenectai/core` package by publishing the generated
...
<Update
  ...
  tags={["Release"]}
>
`@kenectai/lint` can now run **fully in the browser** — validate compositions client-side with no Node.js and no server round-trip — and it now depends only on `@kenectai/parsers`, so installing the validator no longer pulls the studio server or other unrelated packages.

## Features

- **Lint:** New `@kenectai/lint/browser` entry — a node-free, client-side rule engine (`lintHyperframeHtml`, `lintMediaUrls`, `shouldBlockRender`) for browser-only editors and tools ([6aaab32c](https://github.com/heygen-com/hyperframes/commit/6aaab32ccb4de355abca8310c2d4fd69ea95e47b), [#1773](https://github.com/heygen-com/hyperframes/pull/1773))

## Internal

- **Lint:** Depend only on `@kenectai/parsers` (dropped `@kenectai/core`); relocated the shared URL/asset-path, font-alias, and slideshow-parser utilities into `@kenectai/parsers` with back-compat stubs in core, and hardened the CSS `url()` regex against polynomial-ReDoS ([6aaab32c](https://github.com/heygen-com/hyperframes/commit/6aaab32ccb4de355abca8310c2d4fd69ea95e47b), [#1773](https://github.com/heygen-com/hyperframes/pull/1773))
...
- Extract @kenectai/studio-server from core ([7a4853df](https://github.com/heygen-com/hyperframes/commit/7a4853dfe6e7e55effca57314011cec322c13a23), [#1757](https://github.com/heygen-com/hyperframes/pull/1757))
- Extract @kenectai/lint from core ([98d0bdd7](https://github.com/heygen-com/hyperframes/commit/98d0bdd73c7551abeb6962bd82eedc1c44e3d953), [#1756](https://github.com/heygen-com/hyperframes/pull/1756))
- Extract @kenectai/parsers from core ([cdf9c817](https://github.com/heygen-com/hyperframes/commit/cdf9c817e1c0c396a5c24c75ea08a8dba72e3528), [#1755](https://github.com/heygen-com/hyperframes/pull/1755))
...
<Update
  ...
  tags={["Release", "Core"]}
>
This hotfix restores the published `@kenectai/core` subpath contract used by Studio,
...
- **Publish:** Publish @kenectai/sdk to npm ([7607a714](https://github.com/heygen-com/hyperframes/commit/7607a714a6c46407bb42cff1ed754ea8997035df), [#1587](https://github.com/heygen-com/hyperframes/pull/1587))
...
- **Sdk:** Scaffold @kenectai/sdk — engine layer (model, RFC 6902 patches, mutate, apply-patches) ([22bb6737](https://github.com/heygen-com/hyperframes/commit/22bb6737c546e910bfdc0bbe6f459bfe01e6bfa6), [#1324](https://github.com/heygen-com/hyperframes/pull/1324))
- **Core:** Expose hf-ids as subpath export for @kenectai/sdk ([2c64f996](https://github.com/heygen-com/hyperframes/commit/2c64f9969442914a2a8813cc60e5e57646742336), [#1323](https://github.com/heygen-com/hyperframes/pull/1323))
...
>
Adds Google Cloud Run + Cloud Workflows as a distributed-render target (`@kenectai/gcp-cloud-run`) — the GCP counterpart to the AWS Lambda adapter, sharing the same `plan → renderChunk → assemble` primitives. Also lands a font-resolution pipeline so compositions capture and embed their own fonts for deterministic rendering.
