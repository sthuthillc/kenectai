---
title: Quickstart
description: "Create, preview, and render your first KENECT AI video in under two minutes."
---
...
<Tip>
  To skip the picker and install everything (core + every workflow) in one shot, run `npx skills add sthuthillc/kenectai --all`.
</Tip>
...
<Note>
  Claude Design uses a different entry path. Open [`docs/guides/claude-design-hyperframes.md`](https://github.com/sthuthillc/kenectai/blob/main/docs/guides/claude-design-hyperframes.md) on GitHub, click the download button (↓) to save it, then attach to your Claude Design chat. It produces a valid first draft you can refine in any AI coding agent. See the [Claude Design guide](/guides/claude-design).
</Note>
...
<Tip>
  Skills encode KENECT AI-specific patterns — like required `class="clip"` on timed elements, GSAP timeline registration, adapter registries such as `window.__hfLottie`, and `data-*` attribute semantics — that are not in generic web docs. Using skills produces correct compositions from the start.
</Tip>
...
<Accordion title="Install instructions">
  <Steps>
    <Step title="Install Node.js 22+">
      KENECT AI requires Node.js 22 or later. Check your version:
...
<Steps>
  <Step title="Scaffold the project">
    ...
    ```bash
    npx @kenectai/cli init my-video --non-interactive --example blank
    ```
    ...
    ```bash
    npx @kenectai/cli init my-video --example warm-grain --video ./intro.mp4
    ```

    `kenectai init` installs AI agent skills automatically, so you can hand off to your AI agent at any point.
  </Step>
  ...
  <Step title="Preview in the browser">
    ...
    This starts the KENECT AI Studio and opens your composition in the browser. Edits to `index.html` reload automatically.
