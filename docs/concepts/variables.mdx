...
| Frame rate | `--fps` flag on `kenectai render`, or `config.fps` in the SDK |
...
```bash Terminal
# Inline JSON
npx @kenectai/cli render --variables '{"title":"Q4 Report","color":"#1d4ed8"}' --output q4.mp4

# JSON file
npx @kenectai/cli render --variables-file ./vars.json --output out.mp4

# Fail on undeclared or mistyped variables
npx @kenectai/cli render --variables '{"title":"Q4 Report"}' --strict-variables --output out.mp4
```
...
Each row is treated like a `--variables` object and merged over the composition defaults. Output paths support `{key}` placeholders from the row plus `{index}`. KENECT AI validates missing placeholders, output collisions, and `--strict-variables` issues before the first row starts rendering, then writes `manifest.json` next to the outputs with one status row per render.
...
| CLI `--variables` flag | Highest | `kenectai render --variables '{...}'` |
...
If you are building tooling on top of `@kenectai/core`, the variable declarations are readable without rendering:
...
- **Handoff** — copy the effective values as JSON or as a ready-to-run
  `kenectai render --variables` command.
